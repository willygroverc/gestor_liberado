...
if($_POST)
{
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		$sql="INSERT INTO shout_box(user, message, ip_address) value('$username','$message','$user_ip')";
		if(mysql_query($sql)){
		...
		mysql_query($sql_con,"DELETE FROM shout_box WHERE id NOT IN (SELECT * FROM (SELECT id FROM shout_box ORDER BY id DESC LIMIT 0, 10) as sb)");
	}
	elseif($_POST["fetch"]==1)
	{
		$sql_ver = "SELECT user, message, date_time FROM (select * from shout_box ORDER BY id DESC LIMIT 10) shout_box ORDER BY shout_box.id ASC";
		$result=mysql_query($sql_ver);
