...
if (empty($_REQUEST['id_area'])) { $_REQUEST['id_area']="";} else { $_REQUEST['id_area']=$_REQUEST['id_area'];}
?>
<html>
<head>
...
<body>
<div align="center"><form action="" name="form1" method="get">
  <table width="75%" border="1" background="images/fondo.jpg">
    <tr> 
      <th colspan="4"><div align="center"><strong>TERCER NIVEL</strong></div></th>
    </tr>
	<tr>
		<td colspan="4">&nbsp;</td></tr>
    <tr> 
	  ...
	  <select name="txtarea" id="txtarea" onChange="cambiar(this.value)">
		...
		<?php while($area=mysql_fetch_array($datos)){?>
	    	<option value="<?php=$area['area_cod'];?>" <?php if($_REQUEST['id_area']==$area['area_cod']) echo "selected"?>><?php echo $area['area_nombre']; ?></option>
		<?php }?>
      </select></td>
      <td width="21%"><div align="right" class="normal"><strong>Nivel 2:</strong></div></td>
      <td width="29%"> <select name="dominio" id="dominio">
	  <?php 
	  $sql_sen="SELECT * FROM dominio WHERE id_area='$_REQUEST[id_area]'";
	  $data=mysql_db_query($db,$sql_sen,$link);
	  echo "<option value=\"0\">Seleccionar todos</option>";
...
function cambiar(valor){
	dir="imprimir_pre_objetivo.php?id_area="+valor
	self.location=dir
}
-->
</script>