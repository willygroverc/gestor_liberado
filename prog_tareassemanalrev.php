...
{   
	if(isset($action) && $action=="revisar"){
		$sql="UPDATE progtareassemanal1 SET RevisadoPor='$_SESSION[login]', RevisadoPorFecha='".date("Y-m-d H:i:s")."', Aprobacion='$Aprobacion', RevisadoPorObs='$Observaciones', Revisado=1 WHERE IdProgTarea1=$IdProgTarea1";
		mysql_query( $sql);
	}
	else {
		$sql = "SELECT *,  DATE_FORMAT(FechaProceso, '%d/%m/%Y') AS FechaProceso FROM progtareassemanal1 
			    WHERE IdProgTarea='$IdProgTarea'";			
		$res = mysql_query($sql);		
		...
		if ( $sw == 0 )			
		{	$sql="INSERT INTO progtareassemanal1 (IdProgTarea, FechaProceso, RealizadoPor, Realizacion, RealizadoPorObs) VALUES ($IdProgTarea, '".date("Y-m-d H:i:s")."', '$_SESSION[login]', '$Aprobacion', '$Observaciones')";
			mysql_query( $sql);			
		}	
	}
...
<table width="95%" border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
  <tr>
    <th colspan="8" bgcolor="#006699"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif">PROGRAMACION DE TAREAS - SEMANALES</font></th>
  </tr>
  <tr align="center">
  ...
  <?php
		$sql = "SELECT *, DATE_FORMAT(FechaDe, '%d/%m/%Y') AS FechaDe FROM progtareassemanal WHERE IdProgTarea='$IdProgTarea'";
...
if(isset($menu)){
	...
	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM progtareassemanal1 WHERE IdProgTarea='$IdProgTarea' $cond";
	...
	$_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
		...
		$sql = "SELECT *, DATE_FORMAT(FechaProceso, '%d/%m/%Y %H:%i:%s') AS FechaProceso, DATE_FORMAT(RevisadoPorFecha, '%d/%m/%Y %H:%i:%s') AS RevisadoPorFecha 
				FROM progtareassemanal1 WHERE IdProgTarea='$IdProgTarea' $cond ORDER BY IdProgTarea1 DESC LIMIT $_pagi_inicial,$_pagi_cuantos";
		$rs=mysql_query($sql);
		print mysql_error();
		while($tmp=mysql_fetch_array($rs)){
			print "<tr>";
			...
			if($tmp["Revisado"]==1) print "<td align=\"center\"><img src=\"images/ok.gif\" border=\"0\"></td>";
				...
				if($_SESSION[tipo]=="A"  || $tmp['RealizadoPor']!=$_SESSION['login']) print "<td align=\"center\"><a href=\"prog_tareassemanalrev.php?do=revisar&IdProgTarea=$IdProgTarea&IdProgTarea1=$tmp[IdProgTarea1]\"><img src=\"images/no3.gif\" border=\"0\"></a></td>";
...
<form action="prog_tareassemanalrev.php" method="get" name="form2" id="form2" onKeyPress="return Form()">
	<input name="IdProgTarea" type="hidden" value="<?php echo $IdProgTarea;?>">
	<input name="IdProgTarea1" type="hidden" id="IdProgTarea1" value="<?phpecho @$_GET['IdProgTarea1'];?>">
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <table width="80%"  border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
    ...
    <tr align="center">
      <td><?phpecho $lstUsuario[$_SESSION['login']];?></td>
      <td> <font size="2" face="Arial, Helvetica, sans-serif">
        <input name="Aprobacion" type="radio" value="SI" checked <?php if (isset ($row3['Aprobacion']) && $row3['Aprobacion']=="SI") echo "checked";?>>
      SI</font> <font size="2" face="Arial, Helvetica, sans-serif"><br>
&nbsp;
      <input type="radio" name="Aprobacion" value="NO" <?php if (isset($row3['Aprobacion']) && $row3['Aprobacion']=="NO") echo "checked";?>>
      NO</font> <font size="2" face="Arial, Helvetica, sans-serif"><br>
&nbsp;
      <input type="radio" name="Aprobacion" value="NA" <?php if (isset($row3['Aprobacion']) && $row3['Aprobacion']=="NA") echo "checked";?>>
      NA</font> </td>
      <td><font size="2" face="Arial, Helvetica, sans-serif">
        <textarea name="Observaciones" cols="15" id="Observaciones"><?php echo @$row3['Observac'];?></textarea> <!-- REVISAR ESTA VARIABLE NO ESTA DEFINIDA EN BD-->
      </font></td>
    </tr>
    <tr align="center">
      <td height="50" colspan="4">
        <input name="INSERTAR" type="submit" id="INSERTAR" onClick="Mensaje()" value="INSERTAR REALIZACION"<?php print $valid->onSubmit() ?>>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="submit" name="Terminar" value="RETORNAR"></td>
    </tr>
  </table>
...
<script language="JavaScript">
		<!-- 
		<?php if(isset($errorMsg)) print "alert(\"$errorMsg\");";
		?>
function Mensaje()
{ 
...
<?php
	...
	$sw = 0;
	$sql = "SELECT *,  DATE_FORMAT(FechaProceso, '%d/%m/%Y') AS FechaProceso FROM progtareassemanal1 WHERE IdProgTarea='$IdProgTarea'";			
	$res = mysql_query($sql);			
	While ( $fila = mysql_fetch_array($res))
	{	$str = date("d/m/Y");			
		if ( $str == $fila['FechaProceso'] and $_SESSION['login']== $fila['RealizadoPor'])
	...
	$msg =  "El usuario ".$_SESSION['login']." ya registro la Realizacion de Tarea.\\n\\n\tMensaje generado por GesTor F1.";	
	if ($sw == 1 )
	{	print "alert(\"$msg\");";
		print " return false; "; 
	}	
...
function enviar(){
	...
	open("report_tarea.php?IdProgTarea="+IdProgTarea+"&tarea=semanal","TAREAS",'width=590,height=200,status=no,resizable=no,toolbars=no,top=0,left=250,dependent=yes,alwaysRaised=yes')
}
//-->
</script>
