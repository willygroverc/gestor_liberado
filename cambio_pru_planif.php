...
if ($retornar){header("location: lista_prueba_planif.php");}
if ($mas_pruebas)
{	
	$ha=md5($orden);
	$tp_e=explode("_",$tp);
	...
	{
		$fecha_r="$Ano-$Mes-$Dia";
		$sql6="INSERT INTO cambios_prueba_planif (id_orden,fecha_reg,hora_reg,fecha_pru,obj_pru,obs_pru0,obs_pru1,tipo_pru) ".
		"VALUES('$orden','".date("Y-m-d")."','".date("H:i:s")."','$fecha_r','$obj_pru','$obs_pru0','$obs_pru1','$tpo')";
		mysql_db_query($db,$sql6,$link);
		
		$sql_i = "SELECT MAX(id_pru) AS id_pru FROM cambios_prueba_planif";
		$result_i = mysql_db_query($db,$sql_i,$link);
		$row_i = mysql_fetch_array($result_i);
		header("location: cambio_pru2_planif.php?orden=$orden&tp=$tp&num=$row_i[id_pru]");
	}
	else
...
<form name="form1" method="post" onKeyPress="return Form()">
  <table width="95%" border="1" align="center" background="images/fondo.jpg">
    <tr> 
      <th>PLANIFICACION - PRUEBAS 
        <?php	$tp2=md5($orden);
	  		if($tp=="258db52a0b75ba2448af531309ad5eac_".$tp2){echo "USUARIAS"; $tpo="1";}
			elseif($tp=="7d428547b8723786029735cde7b75d7d_".$tp2){echo "DE SISTEMAS"; $tpo="2";}
			elseif($tp=="f4b29b15eee509a8fa39344471561d29_".$tp2){echo "DE SEGURIDAD"; $tpo="3";}
	  ?>
  ...
  <tr> 
    ...
    <tr align="center"> 
      ...
      <td><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">
        <textarea name="obj_pru" cols="50"></textarea>
	...
	<tr> 
	  ...
	  <td width="51%"  > <div align="center"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif"> 
          <textarea name="obs_pru0" cols="50"></textarea>
          </font></div></td>
      <td width="49%"  align="center"><font color="#FFFFFF" size="1" face="Arial, Helvetica">
	   <textarea name="obs_pru1" cols="50"></textarea>
	  </font></td>
    </tr>
   <tr> 
