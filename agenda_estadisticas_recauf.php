...
<?php
$sql_tot="SELECT SUM(recau) AS total_reunion FROM minuta WHERE recau<>0 AND en_fecha BETWEEN '$fec1' AND '$fec2'";
...
<table width="60%" border="1" align="center"  background="images/fondo.jpg">
  <tr> 
    <td width="455"> <table border="1" cellpadding="0" cellspacing="0" width="100%">
		...
		<?php
		$sql="SELECT recau,a.num_codigo FROM `minuta` m, agenda a WHERE m.id_minuta=a.id_agenda AND recau <>0 AND m.en_fecha BETWEEN '$fec1' AND '$fec2'";
		$res=mysql_db_query($db,$sql,$link);
		while($row=mysql_fetch_array($res)){
		?>
        <tr> 
          <td> <div align="left"><font size="2" face="Arial, Helvetica, sans-serif">&nbsp;&nbsp;<?php echo "Reunion ".$row[num_codigo]; ?></font></div></td>
          ...
          <td> <div align="center"><font size="2" face="Arial, Helvetica, sans-serif">&nbsp; 
			  ...
			  $perc=round($row[recau]/$row_tot[total_reunion]*100);
			  echo $perc;
			  ?>
              %</font></div></td>
        ...
        <tr> 
          <th width="237" nowrap bgcolor="#CCCCCC"><font size="2" face="Arial, Helvetica, sans-serif">TOTAL 
            RECAUDADO</font></th>
