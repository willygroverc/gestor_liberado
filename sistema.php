...
function agregarFila($formulario){
	...
	$sCadena = $formulario["Descripcion"];
	$t1Cadena = $formulario["Titular1"];
	$s1Cadena = $formulario["Suplente1"];
	$t2Cadena = $formulario["Titular2"];
	$s2Cadena = $formulario["Suplente2"];
   if (empty($sCadena))
   {   $respuesta->addAlert('El campo SISTEMA, no puede ser vacio');
	   return $respuesta;
   } 
   if (empty($t1Cadena))
   {   $respuesta->addAlert('El campo TITULAR (UN. Sistemas), no puede ser vacio');
	   return $respuesta;
   } 
   if (empty($s1Cadena))
   {   $respuesta->addAlert('El campo SUPLENTE (UN. Sistemas), no puede ser vacio');
	   return $respuesta;
   }
	if (empty($t2Cadena))
   {   $respuesta->addAlert('El campo TITULAR (Dueno), no puede ser vacio');
	   return $respuesta;
   }
   if (empty($s2Cadena))
   {   $respuesta->addAlert('El campo SUPLENTE (Dueno), no puede ser vacio');
	   return $respuesta;
   }
...
function guardar($formulario){
	...
	if($flag=1)
		...
		$MSG="No se han registrados los datos, contactese con soporte.";
    ...
    <tr bgcolor="#006699"> 
		  ...
		  <input type="text" id="Descripcion" name="Descripcion" value="" class="textbox" />
            ...
            <td><div align="center"> 
				...
				<option></option>
                  <?php 
		  $sql_u = "SELECT * FROM users WHERE tipo2_usr='T' AND bloquear=0 ORDER BY apa_usr ASC";
		  $result_u = mysql_query($sql_u);
		  while ($row_u = mysql_fetch_array($result_u)) 
		  {
			echo "<option value=\"$row_u[login_usr]\">$row_u[apa_usr] $row_u[ama_usr] $row_u[nom_usr]</option>";
		  }
		         ?>
                </select>
              </div></td>
