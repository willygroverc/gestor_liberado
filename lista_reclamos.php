...
<form action="" method="get" name="form2" id="form2" >
 <input name="opc" type="hidden" value="<?php echo $opc;?>">
  <table width="80%" border="1" align="center" bgcolor="#006699">
    <tr> 
      <td width="60%" background="windowsvista-assets1/main-button-tile.jpg" height="30"><div align="center"><strong><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif">PUNTO DE RECLAMO - PR</font></strong></div></td>
	    </tr>
  </table>
</form>
...
<table width="100%" border="1" align="center" cellpadding="0" cellspacing="2"  background="images/fondo.jpg">
        <tr> 
          <th colspan="20" background="images/main-button-tileR11.jpg" height="25">RECLAMOS </th>
        </tr>
		<tr align="center"> 
          <td width="30" background="images/main-button-tileR2.jpg" height="25" class="menu">Nro</td>
		  <td width="70" background="images/main-button-tileR2.jpg" class="menu">FECHA </td>
          <td width="70" background="images/main-button-tileR2.jpg" class="menu">NOMBRE</td>
          <td width="200" background="images/main-button-tileR2.jpg" class="menu">RECLAMO<?php //print $help->AddLink("incidencia", "INCIDENCIA"); ?></td>
		  <td width="130" background="images/main-button-tileR2.jpg" class="menu">TIPOLOGIA</td>
          <td width="70" background="images/main-button-tileR2.jpg" class="menu">SUBTIPOLOGIA </td>
          <td width="20" background="images/main-button-tileR2.jpg" class="menu">SOLUCION<?php //print $help->AddLink("segui", "SEGUI"); ?></td>         
          <td width="50" background="images/main-button-tileR2.jpg" class="menu">IMPRIMIR INTERNO</td>
          <td width="60" background="images/main-button-tileR2.jpg" class="menu">ARCHIVO ADJUNTO</td>
		  <td width="60" background="images/main-button-tileR2.jpg" class="menu">ORIGEN 2</td>
        </tr>
        <?php
...
{	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM reclamos";
	...
	$_pagi_totalPags = ceil($row9[pagi_totalReg] / $_pagi_cuantos);
	$_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
	$sql = "SELECT reclamos.* FROM reclamos ORDER BY CReclamo DESC limit $_pagi_inicial,$_pagi_cuantos";}
else if($tipo=="T") 
{	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM reclamos";
	$result9=mysql_db_query($db,$_pagi_sqlConta,$link);
	$row9=mysql_fetch_array($result9);
...
$sql11 = "SELECT * FROM control_parametros";
	...
	$sql = mysql_query("SELECT * FROM reclamos ORDER BY CReclamo DESC LIMIT $inicio, $registros");	
...
function openStat_3() {	
	window.open("exportar_reclamos.php",'Estadìsticas', 'width=610,height=200,status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes');
}
function openStat_4() {	
...
if(mes=-1)
{
if(confirm("Esta Seguro de Exportar los Reclamos Correspondiente al Mes 12 del año "+anioo+""))
{
}
else
{
...
{
if(confirm("Esta Seguro de Exportar los Reclamos Correspondiente al Mes "+mes+" del año "+anio+""))
{
}
else
{
