...
$typeRespAct = 3;
$listResAct = array (
	"Computadores de Escritorio"=>"Computadores de Escritorio",
	"Computadores Portatiles"=>"Computadores Portatiles",
	"Servidores"=>"Servidores",
	"Varios"=>"Varios",
);
$listEscritorio= $list -> GetTable4(1);
$listPortatiles = $list -> GetTable4(2);
$listServidores = $list -> GetTable4(3);
$listVarios = $list -> GetTable4(4);
if (!$listEscritorio) $listEscritorio[0]="     ";
if (!$listPortatiles) $listPortatiles[0]="     ";
if (!$listServidores) $listServidores[0]="     ";
if (!$listVarios) $listVarios[0]="     ";
$listUser = $listEscritorio;
// edicion del registro
if ($action == "edit") {
    $tmp = $prueba -> GetDetail(0, 0 , $id_pruresin);
    $recordDb = $tmp[0];
	$action="update";
	if ($recordDb["resact"]=="Computadores Portatiles") $listUser=$listPortatiles;
	elseif ($recordDb["resact"]=="Servidores") $listUser=$listServidores;
	elseif ($recordDb["resact"]=="Varios") $listUser=$listVarios;
	else  $listUser=$listEscritorio;
...
<?php
	$i=0;
	foreach ($listEscritorio as $k => $v){
		print "group[0][$i]=new Option(\"$v\",\"$k\")\n";
		$i++;
	}
		$i=0;
	foreach ($listPortatiles as $k => $v){
		print "group[1][$i]=new Option(\"$v\",\"$k\")\n";
		$i++;
	}
	$i=0;
	foreach ($listServidores as $k => $v){
