...
<table width="100%" border="1">
  <tr align="center"> 
	...
	if($selec=="Tarea" OR $selec=="")
	{
		...
		$sql_ta="SELECT HoraDe, HoraA, sm_asig, Actividad, IdProgTarea, Dia, Mes FROM progtareassemestral WHERE IdProgTarea IN($lista) ORDER BY Mes ASC, Dia ASC, HoraDe ASC";
	}
	if($selec=="Asignado")
	{
		$lista=str_replace("|*|",",",$asignado);
		$lista=str_replace("@","'",$lista);
		$sql_ta="SELECT HoraDe, HoraA, sm_asig, Actividad, IdProgTarea, Dia, Mes FROM progtareassemestral WHERE sm_asig IN($lista) ORDER BY Mes ASC, Dia ASC, HoraDe ASC";	
	}
	...
	{
		$nom_resp=nom_comp($row_ta2[sm_asig]);
		if($row_ta2[sm_asig]=="0"){echo "<td width=".$porc."%><strong><font color=\"#000000\" size=\"1\" face=\"Arial, Helvetica, sans-serif\">Todos</font></strong></td>";}
		else
	...
	for($lim=0;$lim<$num_meses;$lim++)
  	{
		...
		{
			case "1": 
				$num_dias="30"; $nom_mes="Ene, Feb, Mar, Abr, May, Jun"; $mes_ini="01"; $mes_fin="06"; 
			break;
			case "2": 
				$num_dias="31"; $nom_mes="Jul, Ago, Sep, Oct, Nov, Dic"; $mes_ini="07"; $mes_fin="12";
			break;
		}
  ...
  {
		echo "<tr>";
		echo "<td><div align=\"center\"><strong><font color=\"#000000\" size=\"1\" face=\"Arial, Helvetica, sans-serif\">Semestre $meses[$lim]<br>$nombre_mes[$lim]</font></strong></div></td>";
		for($fer=1;$fer<=$row_ta0;$fer++)
		{
			$sql_ta6="SELECT RealizadoPor, RevisadoPor, DATE_FORMAT(FechaProceso,'%d/%m/%Y %H:%i:%s') as FechaProceso, DATE_FORMAT(RevisadoPorFecha,'%d/%m/%Y %H:%i:%s') as RevisadoPorFecha FROM progtareassemestral1 WHERE IdProgTarea='$id_prog[$fer]' AND FechaProceso BETWEEN '$dia_ini[$lim]' AND '$dia_fin[$lim]'";
