...
elseif (isset($_REQUEST['reg_form']))
	{	include("conexion.php");
		...
		$sql6= "SELECT * FROM solicproycontrol WHERE Codigo='$var' AND Responsabilid='$Responsabilid'";
		$result6=mysql_query($sql6);
		$row6=mysql_fetch_array($result6);
		
		if(!$row6['Codigo'] && !$row6['Responsabilid'])
		{	$sql="INSERT INTO ".
			"solicproycontrol (Codigo,Responsabilid,Actividades,Aprobacion,Observac) ".
			"VALUES ('$var','$Responsabilid','$Actividades','$Aprobacion','$Observac')";
			mysql_query($sql);
			header("location: solicproyecto5_last.php?Codigo=$var");
		}
		else
		{	$sql="UPDATE solicproycontrol SET Actividades='$Actividades',Aprobacion='$Aprobacion',".
...
$valid->addIsTextNormal ( "Actividades",  "Control, $errorMsgJs[expresion]" );
      ...
      <td> 
        <table width="100%" border="1" cellpadding="0" cellspacing="0" bgcolor="#006699">
          <tr> 
            <td><div align="center"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif"><strong>SOLICITUD 
                DE PROYECTOS - MODIFICACION FASE DE CONTROL</strong></font></div></td>
          </tr>
        </table>
        ...
        <table width="100%" border="1" align="center" cellpadding="1" cellspacing="2" background="images/fondo.jpg">
          <tr> 
            ...
            <th width="300" rowspan="2" nowrap bgcolor="#006699"><p><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif">CONTROL</font><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><br>
		...
		while($row=mysql_fetch_array($result)) 
  		{
		 ?>
          <tr align="center"> <?php echo "<td><a href=\"solicproyecto5_last.php?Codigo=$Codigo&Respon=".$row['Responsabilid']."\">".$row['Responsabilid']."</a></td>";?> 
          ...
          <tr> 
            <td height="7" nowrap><div align="center"><strong> </strong><strong> 
                ...
                <select name="Responsabilid">
                  ...
                  <?php 
			  $sql4 = "SELECT *,DATE_FORMAT(Fecha_cont, '%d/%m/%Y') AS Fecha_cont FROM solicproycontrol WHERE Codigo='$Codigo' AND Responsabilid='$Respon'";
            ...
            <td width="279" height="7" nowrap><div align="center"> 
                <textarea name="Observac" cols="35"><?php echo $row4['Observac']?></textarea>
              </div></td>
            <td width="52" nowrap><div align="center"> 
                <?php if (!isset($Respon)){echo date("d/m/Y");} else {echo $row4['Fecha_cont'];}?>
              </div></td>
          </tr>
          <tr> 
