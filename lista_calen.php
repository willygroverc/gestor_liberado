...
if(isset($varia3) && $varia3=="creacion"){
	...
	$sql3 = "INSERT INTO ordenes (fecha, time, cod_usr, desc_inc, tipo,nomb_archivo,area,dominio,objetivo,ci_ruc,id_anidacion,origen,hash_archivo,observaciones) ".
			"VALUES('".date("Y-m-d")."','".date("H:i:s")."','".$login."','$accion_obj','L','','','','','','','2.3','','')"; 
	mysql_query($sql3);
	$sql_aux="SELECT MAX(id_orden) AS id FROM ordenes";
	$result_aux=mysql_query($sql_aux);
	$fila_aux=mysql_fetch_array($result_aux);
	$idmant = $fila_aux['id'];
	$sql4="UPDATE calenmantplanif SET orden='$idmant' WHERE id_cmant='$variacua'"; 
	mysql_query($sql4);
...
<form name="form2" method="post" action="" onKeyPress="return Form()">
  <table width="80%" align="center" bgcolor="#006699">
    <tr> 
      <td background="windowsvista-assets1/main-button-tile.jpg" height="30"><div align="center"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong>Busqueda 
          de calendarizacion por :</strong></font> 
          <select name="campo">
            <option value="AdicUSI">CODIGO ADICIONAL</option>
          </select>
          &nbsp;&nbsp;&nbsp; 
          <input name="busqueda" type="text" size="40" value="<?php if (isset($busqueqda)) echo $busqueda;?>">
          &nbsp;&nbsp;&nbsp;&nbsp; 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#006699"  background="images/fondo.jpg">
  <tr> 
    <td height="69" valign="top"> <table width="100%" border="1" align="center" cellpadding="0" cellspacing="2" background="images/fondo.jpg" >
        <tr> 
          <th colspan="10" background="windowsvista-assets1/main-button-tile.jpg" height="30">CRONOGRAMA DE MANTENIMIENTO</th>
        </tr>
        <tr align=\"center\"> 
          <th width="3%" class="menu" background="images/main-button-tileR1.jpg">Nro</th>
          <th width="10%" class="menu" background="images/main-button-tileR1.jpg">CODIGO ADICIONAL</th>
          <th width="13%" class="menu" background="images/main-button-tileR1.jpg">ASIGNADO A</th>
          <th width="7%" class="menu" background="images/main-button-tileR1.jpg">ESTADO</th>
          <th width="10%" class="menu" background="images/main-button-tileR1.jpg">FECHA INICIO</th>
          <th width="10%" class="menu" background="images/main-button-tileR1.jpg">FECHA FINAL</th>
		  <th width="16%" class="menu" background="images/main-button-tileR1.jpg">GARANTIA - MANTENIMIENTO:</th>
		  <th width="12%" class="menu" background="images/main-button-tileR1.jpg">SEGUIMIENTO</th>
          <th width="10%" class="menu" background="images/main-button-tileR1.jpg" wdith="2%">IMPRIMIR</th>
		  <th width="9%" class="menu" background="images/main-button-tileR1.jpg">ORDEN DE TRABAJO</th>
        </tr>
        <?php
$fechahoy=date("Y-m-d");
if (isset($BUSCAR)){$sql3 = "SELECT * FROM calenmantplanif a, datfichatec b WHERE a.AdicUSI=b.AdicUSI AND b.Elim<>1 AND a.$campo LIKE '%$busqueda%' GROUP BY a.AdicUSI ASC";}
	...
	while ($row=mysql_fetch_array($result)) {
		...
		{	
			...
			echo "<td><table border='0'>";
				if(isset($rowcont['cont_manten'])){
						...
						if($rowf['fecha_al']){$colorA="bgcolor=\"#00CCFF\"";}
							...
							echo "<tr>
									...
									<td><a href=\"contrato_garantia_last.php?idTabla=$row[id_cmant]&IdContra=".$row['AdicUSI']."\">&nbsp;$rowcont[FechAl2a]</a></td>
								  </tr>";
				}
				if(isset($rowcont['cont_garantia'])){
				$sqlau="SELECT *, DATE_FORMAT(GarantAl, '%d/%m/%Y') AS fecha2 FROM datfichatec WHERE AdicUSI='$row[AdicUSI]'";
				$resultau=mysql_query($sqlau);
				...
				$resultf=mysql_query($sqlf);
				$rowf=mysql_fetch_array($resultf);
					if(isset($rowf['GarantAl'])){$colorB="bgcolor=\"#CCCCCC\"";}
					else{$colorB="bgcolor=\"#666666\"";}
				echo "<tr>
					<td $colorB><div align='right'>Garantia :</div></td>
					<td><a href=\"contrato_garantia_last.php?idTabla=$row[id_cmant]&IdContra=".$row['AdicUSI']."\">&nbsp;$rowau[fecha2]</a></td>
				  </tr>";}
				if(!isset($rowcont['cont_garantia']) && !isset($rowcont['cont_manten'])){echo "<td><a href=\"contrato_garantia_last.php?idTabla=$row[id_cmant]&IdContra=".$row[AdicUSI]."\">NINGUNO</a></td>";}
				echo "</table></td>";
			}else{
				echo "<td><a href=\"contrato_garantia.php?idTabla=$row[id_cmant]&IdContra=".$row['AdicUSI']."\">NINGUNO</a></td>";}
		...
		{	$color="bgcolor=\"#FFFF00\"";	
			...
			//Nuevo código
			$sqla = "select *from datfichatec where AdicUSI = '$row3[AdicUSI]'";
			$resa = mysql_query($sqla);
			$rowa = mysql_fetch_array($resa);
			...
			$rowcont=mysql_fetch_array($resultcont);
			if($rowcont){
				if($rowcont['cont_garantia']=='1'){$colorA="bgcolor=\"#FF9999\"";}
				elseif($rowcont['mantenimiento']=='1'){$colorA="bgcolor=\"#00FFFF\"";}
				else{$colorA="bgcolor=\"#FFFFFF\"";}
				echo "<td ".$colorA."><a href=\"contrato_garantia_last.php?idTabla=$row5[id_cmant]&IdContra=".$row5['AdicUSI']."\">&nbsp;$rowcont[FechAl2a]</a></td>";
			}else{
				echo "<td><a href=\"contrato_garantia.php?idTabla=$row5[id_cmant]&IdContra=".$row5['AdicUSI']."\">NINGUNO</a></td>";}
