...
// PHP Report Maker 1.0 - Table level configuration (Reporte de Tipificacion por Mes)
// Table Level Constants

define("EW_TABLE_VAR", "Reporte_de_Tipificacion_por_Mes", TRUE);
define("EW_TABLE_GROUP_PER_PAGE", "grpperpage", TRUE);
define("EW_TABLE_SESSION_GROUP_PER_PAGE", "Reporte_de_Tipificacion_por_Mes_grpperpage", TRUE);
define("EW_TABLE_START_GROUP", "start", TRUE);
define("EW_TABLE_SESSION_START_GROUP", "Reporte_de_Tipificacion_por_Mes_start", TRUE);
define("EW_TABLE_SESSION_SEARCH", "Reporte_de_Tipificacion_por_Mes_search", TRUE);
define("EW_TABLE_CHILD_USER_ID", "childuserid", TRUE);
define("EW_TABLE_SESSION_CHILD_USER_ID", "Reporte_de_Tipificacion_por_Mes_childuserid", TRUE);
...
$EW_TABLE_SQL_SELECT = "SELECT COUNT(*) AS aux, ordenes.id_orden, ordenes.fecha, ordenes.`time`, ordenes.cod_usr, ordenes.desc_inc, `area`.area_nombre, dominio.dominio, month(ordenes.fecha) AS mes, objetivos.objetivo FROM " . EW_TABLE_SQL_FROM;
$EW_TABLE_SQL_WHERE = "";
define("EW_TABLE_SQL_GROUPBY", "ordenes.id_orden, ordenes.fecha, ordenes.`time`, ordenes.cod_usr, ordenes.desc_inc, `area`.area_nombre, dominio.dominio, month(ordenes.fecha), objetivos.objetivo", TRUE);
define("EW_TABLE_SQL_HAVING", "", TRUE);
define("EW_TABLE_SQL_ORDERBY", "month(ordenes.fecha) ASC, `area`.area_nombre ASC, dominio.dominio ASC, `objetivo` ASC", TRUE);
...
$Ordenes_por_Mes_cht_parms = NULL; // Store all chart parameters

// Chart data
$Ordenes_por_Mes_cht_index = NULL;
$Ordenes_por_Mes_cht_id = NULL;
$Ordenes_por_Mes_cht_smry = NULL;
$Ordenes_por_Mes_cht_XFld = NULL;
$Ordenes_por_Mes_cht_YFld = NULL;
$Ordenes_por_Mes_cht_YFldBase = NULL;
$Ordenes_por_Mes_cht_XFld = "mes";
$Ordenes_por_Mes_cht_YFld = "aux";
$Ordenes_por_Mes_cht_XDateFld = "";
?>
<?php
...
<a name="top"></a>
<?php } ?>
Reporte de Tipificacion por Mes
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="Reporte_de_Tipificacion_por_Messmry.php?export=html">Imprimir</a>
&nbsp;&nbsp;<a href="Reporte_de_Tipificacion_por_Messmry.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;<a href="Reporte_de_Tipificacion_por_Messmry.php?export=word">Exportar a Word</a>
<?php } ?>
<br><br>
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php if (@$sExport == "") { ?>
<form action="Reporte_de_Tipificacion_por_Messmry.php" name="ewpagerform" id="ewpagerform">
...
<!--first page button-->
	...
	<?php } else { ?>
	<td><a href="Reporte_de_Tipificacion_por_Messmry.php?start=1"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($PrevStart == $nStartGrp) { ?>
	<td><img src="rptimages/prevdisab.gif" alt="Previous" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Reporte_de_Tipificacion_por_Messmry.php?start=<?php echo $PrevStart; ?>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--current page number-->
...
<!--next page button-->
	...
	<?php } else { ?>
	<td><a href="Reporte_de_Tipificacion_por_Messmry.php?start=<?php echo $NextStart; ?>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>
	<?php  } ?>
<!--last page button-->
	<?php if ($LastStart == $nStartGrp) { ?>
	<td><img src="rptimages/lastdisab.gif" alt="Last" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Reporte_de_Tipificacion_por_Messmry.php?start=<?php echo $LastStart; ?>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>
	<?php } ?>
...
<?php
	...
	if (intval($nGrpCount) >= intval($nStartGrp) && $nGrpCount <= $nStopGrp) {
		...
		if (($x_area_nombre == NULL && $ox_area_nombre == NULL) ||
			(($x_area_nombre <> "" && $ox_area_nombre == $dgx_area_nombre) && !ChkLvlBreak(2))) {
			$dgx_area_nombre = "";
		...
		if (($x_dominio == NULL && $ox_dominio == NULL) ||
			(($x_dominio <> "" && $ox_dominio == $dgx_dominio) && !ChkLvlBreak(3))) {
			$dgx_dominio = "";
		...
		if (($x_objetivo == NULL && $ox_objetivo == NULL) ||
			(($x_objetivo <> "" && $ox_objetivo == $dgx_objetivo) && !ChkLvlBreak(4))) {
			$dgx_objetivo = "";
...
<tr><td colspan="3" class="ewPadding"><div id="ewBottom" class="phpreportmaker">
	<!-- Bottom slot -->
<a name="cht_Ordenes_por_Mes"></a>
<div id="div_Ordenes_por_Mes">
<?php

// Initialize chart data
$Ordenes_por_Mes_cht_id = "Reporte_de_Tipificacion_por_Mes_Ordenes_por_Mes"; // Chart ID
$Ordenes_por_Mes_cht_parms .= "type=1,"; // Chart type
$Ordenes_por_Mes_cht_parms .= "bgcolor=#FCFCFC,"; // Background color
$Ordenes_por_Mes_cht_parms .= "caption=" . ew_Encode("Ordenes por Mes") . ","; // Chart caption
$Ordenes_por_Mes_cht_parms .= "borderleftcolor=#808080,"; // Border left color
$Ordenes_por_Mes_cht_parms .= "bordertopcolor=#808080,"; // Border top color
$Ordenes_por_Mes_cht_parms .= "borderbottomcolor=#808080,"; // Border bottom color
$Ordenes_por_Mes_cht_parms .= "borderrightcolor=#808080,"; // Border right color
$Ordenes_por_Mes_cht_parms .= "chartbgcolor=#EEEEEE,"; // Chart background color
$Ordenes_por_Mes_cht_parms .= "chartbordercolor=#A9A9A9,"; // Chart border color
$Ordenes_por_Mes_cht_parms .= "numgridlines=3,"; // Number of grid lines
$Ordenes_por_Mes_cht_parms .= "gridlinecolor=#DCDCDC,"; // Grid line color
$Ordenes_por_Mes_cht_parms .= "xaxisname=" . ew_Encode("Mes") . ","; // X axis name
$Ordenes_por_Mes_cht_parms .= "xaxisnamerotated=0,"; // X axis name rotated
$Ordenes_por_Mes_cht_parms .= "xaxisvaluemaxchar=20,"; // X axis value max char
$Ordenes_por_Mes_cht_parms .= "yaxisname=" . ew_Encode("Numero de Ordenes de Trabajo") . ","; // Y axis name
$Ordenes_por_Mes_cht_parms .= "yaxisnamerotated=1,"; // Y axis name rotated
$Ordenes_por_Mes_cht_parms .= "shownames=1,"; // Show names
$Ordenes_por_Mes_cht_parms .= "showvalues=1,"; // Show values
$Ordenes_por_Mes_cht_parms .= "showhover=0,"; // Show hover
$Ordenes_por_Mes_cht_parms .= "alpha=50,"; // Chart alpha
$Ordenes_por_Mes_cht_parms .= "colorpalette=#FF0000|#FF0080|#FF00FF|#8000FF|#FF8000|#FF3D3D|#7AFFFF|#0000FF|#FFFF00|#FF7A7A|#3DFFFF|#0080FF|#80FF00|#00FF00|#00FF80|#00FFFF,"; // Chart color palette
$Ordenes_por_Mes_cht_parms .= "cssurl=rptcss/ewchart.css"; // Chart css
ew_SortChartData($Ordenes_por_Mes_cht_smry, 0);
echo ew_ShowChart($Ordenes_por_Mes_cht_id, $Ordenes_por_Mes_cht_parms, $Ordenes_por_Mes_cht_smry, 550, 440, "");
?>
</div>
<a href="#top">Top</a>
<br><br>
	...
	switch ($lvl) {
		...
		case 2:
			return ($GLOBALS["x_area_nombre"] == NULL && $GLOBALS["ox_area_nombre"] != NULL) ||
				...
				($GLOBALS["x_area_nombre"] <> $GLOBALS["ox_area_nombre"]) || ChkLvlBreak(1); // Recurse upper level
		case 3:
			return ($GLOBALS["x_dominio"] == NULL && $GLOBALS["ox_dominio"] != NULL) ||
				($GLOBALS["x_dominio"] !== NULL && $GLOBALS["ox_dominio"] == NULL) ||
				($GLOBALS["x_dominio"] <> $GLOBALS["ox_dominio"]) || ChkLvlBreak(2); // Recurse upper level
		case 4:
			return ($GLOBALS["x_objetivo"] == NULL && $GLOBALS["ox_objetivo"] != NULL) ||
				($GLOBALS["x_objetivo"] !== NULL && $GLOBALS["ox_objetivo"] == NULL) ||
				($GLOBALS["x_objetivo"] <> $GLOBALS["ox_objetivo"]) || ChkLvlBreak(3); // Recurse upper level
	}
}
...
function ResetLevelSummary($lvl) {
	...
	if ($lvl <= 2) $GLOBALS["ox_area_nombre"] = "";
	if ($lvl <= 3) $GLOBALS["ox_dominio"] = "";
	if ($lvl <= 4) $GLOBALS["ox_objetivo"] = "";
...
function ProcessChartData(&$rs) {
	AccumulateChartSummary($rs, $GLOBALS["Ordenes_por_Mes_cht_smry"], $GLOBALS["Ordenes_por_Mes_cht_XFld"], "", $GLOBALS["Ordenes_por_Mes_cht_YFld"], 0, "", "SUM");
}
?>
<?php
