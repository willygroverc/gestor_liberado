...
// PHP Report Maker 1.0 - Table level configuration (Planificacion de Pruebas)
// Table Level Constants

define("EW_TABLE_VAR", "Planificacion_de_Pruebas", TRUE);
define("EW_TABLE_GROUP_PER_PAGE", "grpperpage", TRUE);
define("EW_TABLE_SESSION_GROUP_PER_PAGE", "Planificacion_de_Pruebas_grpperpage", TRUE);
define("EW_TABLE_START_GROUP", "start", TRUE);
define("EW_TABLE_SESSION_START_GROUP", "Planificacion_de_Pruebas_start", TRUE);
define("EW_TABLE_SESSION_SEARCH", "Planificacion_de_Pruebas_search", TRUE);
define("EW_TABLE_CHILD_USER_ID", "childuserid", TRUE);
define("EW_TABLE_SESSION_CHILD_USER_ID", "Planificacion_de_Pruebas_childuserid", TRUE);

// Table Level SQL
define("EW_TABLE_SQL_FROM", "ordenes RIGHT OUTER JOIN asignacion ON (ordenes.id_orden = asignacion.id_asig) LEFT OUTER JOIN resulpru ON (asignacion.id_orden = resulpru.ordayuda) LEFT OUTER JOIN planprueba ON (asignacion.id_orden = planprueba.ordayuda)", TRUE);
$EW_TABLE_SQL_SELECT = "SELECT ordenes.id_orden, ordenes.fecha, ordenes.`time`, ordenes.desc_inc, planprueba.fecplanif FROM " . EW_TABLE_SQL_FROM;
$EW_TABLE_SQL_WHERE = "(asignacion.area = 'contingencia')";
...
$af_fecplanif = NULL; // Advanced filter for fecplanif
?>
<?php
...
$x_fecplanif = NULL; $ox_fecplanif = NULL; $tx_fecplanif = NULL; $ftx_fecplanif = 133; $rf_fecplanif = NULL; $rt_fecplanif = NULL;
?>
<?php
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="Planificacion_de_Pruebassmry.php?export=html">Imprimir</a>
&nbsp;&nbsp;<a href="Planificacion_de_Pruebassmry.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;<a href="Planificacion_de_Pruebassmry.php?export=word">Exportar a Word</a>
<?php } ?>
<br><br>
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php if (@$sExport == "") { ?>
<form action="Planificacion_de_Pruebassmry.php" name="ewpagerform" id="ewpagerform">
...
<!--first page button-->
	...
	<?php } else { ?>
	<td><a href="Planificacion_de_Pruebassmry.php?start=1"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($PrevStart == $nStartGrp) { ?>
	<td><img src="rptimages/prevdisab.gif" alt="Previous" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Planificacion_de_Pruebassmry.php?start=<?php echo $PrevStart; ?>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--current page number-->
...
<!--next page button-->
	...
	<?php } else { ?>
	<td><a href="Planificacion_de_Pruebassmry.php?start=<?php echo $NextStart; ?>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>
	<?php  } ?>
<!--last page button-->
	<?php if ($LastStart == $nStartGrp) { ?>
	<td><img src="rptimages/lastdisab.gif" alt="Last" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Planificacion_de_Pruebassmry.php?start=<?php echo $LastStart; ?>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>
	<?php } ?>
...
<?php echo str_replace(chr(10), "<br>", $x_desc_inc); ?>
		...
		<td class="ewRptDtlField">
<?php echo ew_FormatDateTime($x_fecplanif,5) ?>
</td>
	</tr>
<?php
...
<?php if ($nTotalGrps > 0) { ?>
	...
	<tr class="ewRptGrandSummary"><td colspan="5">Total Registros (<?php echo ew_FormatNumber($cnt[0][0],0,-2,-2,-2); ?> Registros Detalldos)</td></tr>
	<!--tr><td colspan="5"><span class="phpreportmaker">&nbsp;<br></span></td></tr-->
<?php } ?>
</table>
<!-- </form> -->
...
function GetRow($opt) {
	...
	while ($row) {
		if (ValidRow($row)) {
			...
			$GLOBALS["x_fecplanif"] = $row["fecplanif"];
			$val[5] = $GLOBALS["x_fecplanif"];
			break;
		} else {
			$rsidx++;
			$row = ($rsidx < $rscnt) ? $rs[$rsidx] : FALSE;
