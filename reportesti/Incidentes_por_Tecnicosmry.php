...
// PHP Report Maker 1.0 - Table level configuration (Incidentes por Tecnico)
// Table Level Constants

define("EW_TABLE_VAR", "Incidentes_por_Tecnico", TRUE);
define("EW_TABLE_GROUP_PER_PAGE", "grpperpage", TRUE);
define("EW_TABLE_SESSION_GROUP_PER_PAGE", "Incidentes_por_Tecnico_grpperpage", TRUE);
define("EW_TABLE_START_GROUP", "start", TRUE);
define("EW_TABLE_SESSION_START_GROUP", "Incidentes_por_Tecnico_start", TRUE);
define("EW_TABLE_SESSION_SEARCH", "Incidentes_por_Tecnico_search", TRUE);
define("EW_TABLE_CHILD_USER_ID", "childuserid", TRUE);
define("EW_TABLE_SESSION_CHILD_USER_ID", "Incidentes_por_Tecnico_childuserid", TRUE);

// Table Level SQL
define("EW_TABLE_SQL_FROM", "users RIGHT OUTER JOIN ordenes ON (users.login_usr = ordenes.cod_usr)", TRUE);
$EW_TABLE_SQL_SELECT = "SELECT count(*) AS total, ordenes.id_orden, ordenes.fecha, ordenes.`time`, users.login_usr, ordenes.desc_inc, users.tipo2_usr FROM " . EW_TABLE_SQL_FROM;
$EW_TABLE_SQL_WHERE = "(users.tipo2_usr = 'T')";
define("EW_TABLE_SQL_GROUPBY", "ordenes.id_orden, ordenes.fecha, ordenes.`time`, users.login_usr, ordenes.desc_inc, users.tipo2_usr", TRUE);
define("EW_TABLE_SQL_HAVING", "", TRUE);
define("EW_TABLE_SQL_ORDERBY", "users.login_usr ASC", TRUE);
...
$af_login_usr = NULL; // Advanced filter for login_usr
...
// Group variables
$x_login_usr = NULL; $ox_login_usr = NULL; $gx_login_usr = NULL; $dgx_login_usr = NULL; $tx_login_usr = NULL; $ftx_login_usr = 200; $gfx_login_usr = $ftx_login_usr; $gbx_login_usr = ""; $gix_login_usr = "0"; $rf_login_usr = NULL; $rt_login_usr = NULL;
...
$Incidente_por_Tecnico_cht_parms = NULL; // Store all chart parameters

// Chart data
$Incidente_por_Tecnico_cht_index = NULL;
$Incidente_por_Tecnico_cht_id = NULL;
$Incidente_por_Tecnico_cht_smry = NULL;
$Incidente_por_Tecnico_cht_XFld = NULL;
$Incidente_por_Tecnico_cht_YFld = NULL;
$Incidente_por_Tecnico_cht_YFldBase = NULL;
$Incidente_por_Tecnico_cht_XFld = "login_usr";
$Incidente_por_Tecnico_cht_YFld = "total";
$Incidente_por_Tecnico_cht_XDateFld = "";
?>
<?php
...
<a name="top"></a>
<?php } ?>
Incidentes por Tecnico
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="Incidentes_por_Tecnicosmry.php?export=html">Imprimir</a>
&nbsp;&nbsp;<a href="Incidentes_por_Tecnicosmry.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;<a href="Incidentes_por_Tecnicosmry.php?export=word">Exportar a Word</a>
<?php } ?>
<br><br>
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php if (@$sExport == "") { ?>
<form action="Incidentes_por_Tecnicosmry.php" name="ewpagerform" id="ewpagerform">
...
<!--first page button-->
	...
	<?php } else { ?>
	<td><a href="Incidentes_por_Tecnicosmry.php?start=1"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($PrevStart == $nStartGrp) { ?>
	<td><img src="rptimages/prevdisab.gif" alt="Previous" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Incidentes_por_Tecnicosmry.php?start=<?php echo $PrevStart; ?>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--current page number-->
...
<!--next page button-->
	...
	<?php } else { ?>
	<td><a href="Incidentes_por_Tecnicosmry.php?start=<?php echo $NextStart; ?>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>
	<?php  } ?>
<!--last page button-->
	<?php if ($LastStart == $nStartGrp) { ?>
	<td><img src="rptimages/lastdisab.gif" alt="Last" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Incidentes_por_Tecnicosmry.php?start=<?php echo $LastStart; ?>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>
	<?php } ?>
...
<?php
	...
	if (intval($nGrpCount) >= intval($nStartGrp) && $nGrpCount <= $nStopGrp) {
		...
		// Show group values
		$dgx_login_usr = $x_login_usr;
		if (($x_login_usr == NULL && $ox_login_usr == NULL) ||
			(($x_login_usr <> "" && $ox_login_usr == $dgx_login_usr) && !ChkLvlBreak(1))) {
			$dgx_login_usr = "";
		} elseif ($x_login_usr == NULL) {
			$dgx_login_usr = EW_NULL_LABEL;
		} elseif ($x_login_usr == "") {
			$dgx_login_usr = EW_EMPTY_LABEL;
		}
?>
	<tr<?php echo $sItemRowClass; ?>>
		<td class="ewRptGrpField1">
		<?php $tx_login_usr = $x_login_usr; $x_login_usr = $dgx_login_usr; ?>
<?php echo $x_login_usr ?>
		<?php $x_login_usr = $tx_login_usr; ?></td>
...
<tr><td colspan="3" class="ewPadding"><div id="ewBottom" class="phpreportmaker">
	<!-- Bottom slot -->
<a name="cht_Incidente_por_Tecnico"></a>
<div id="div_Incidente_por_Tecnico">
<?php

// Initialize chart data
$Incidente_por_Tecnico_cht_id = "Incidentes_por_Tecnico_Incidente_por_Tecnico"; // Chart ID
$Incidente_por_Tecnico_cht_parms .= "type=2,"; // Chart type
$Incidente_por_Tecnico_cht_parms .= "bgcolor=#FCFCFC,"; // Background color
$Incidente_por_Tecnico_cht_parms .= "caption=" . ew_Encode("Incidente por Tecnico") . ","; // Chart caption
$Incidente_por_Tecnico_cht_parms .= "borderleftcolor=#808080,"; // Border left color
$Incidente_por_Tecnico_cht_parms .= "bordertopcolor=#808080,"; // Border top color
$Incidente_por_Tecnico_cht_parms .= "borderbottomcolor=#808080,"; // Border bottom color
$Incidente_por_Tecnico_cht_parms .= "borderrightcolor=#808080,"; // Border right color
$Incidente_por_Tecnico_cht_parms .= "chartbgcolor=#EEEEEE,"; // Chart background color
$Incidente_por_Tecnico_cht_parms .= "chartbordercolor=#A9A9A9,"; // Chart border color
$Incidente_por_Tecnico_cht_parms .= "numgridlines=3,"; // Number of grid lines
$Incidente_por_Tecnico_cht_parms .= "gridlinecolor=#DCDCDC,"; // Grid line color
$Incidente_por_Tecnico_cht_parms .= "xaxisname=" . ew_Encode("Nombre del Tecnico") . ","; // X axis name
$Incidente_por_Tecnico_cht_parms .= "xaxisnamerotated=0,"; // X axis name rotated
$Incidente_por_Tecnico_cht_parms .= "xaxisvaluemaxchar=20,"; // X axis value max char
$Incidente_por_Tecnico_cht_parms .= "yaxisname=" . ew_Encode("Total Incidentes") . ","; // Y axis name
$Incidente_por_Tecnico_cht_parms .= "yaxisnamerotated=1,"; // Y axis name rotated
$Incidente_por_Tecnico_cht_parms .= "shownames=1,"; // Show names
$Incidente_por_Tecnico_cht_parms .= "showvalues=1,"; // Show values
$Incidente_por_Tecnico_cht_parms .= "showhover=1,"; // Show hover
$Incidente_por_Tecnico_cht_parms .= "alpha=50,"; // Chart alpha
$Incidente_por_Tecnico_cht_parms .= "colorpalette=#FF0000|#FF0080|#FF00FF|#8000FF|#FF8000|#FF3D3D|#7AFFFF|#0000FF|#FFFF00|#FF7A7A|#3DFFFF|#0080FF|#80FF00|#00FF00|#00FF80|#00FFFF,"; // Chart color palette
$Incidente_por_Tecnico_cht_parms .= "cssurl=rptcss/ewchart.css"; // Chart css
ew_SortChartData($Incidente_por_Tecnico_cht_smry, 0);
echo ew_ShowChart($Incidente_por_Tecnico_cht_id, $Incidente_por_Tecnico_cht_parms, $Incidente_por_Tecnico_cht_smry, 550, 440, "");
?>
</div>
<a href="#top">Top</a>
<br><br>
...
function ChkLvlBreak($lvl) {
	switch ($lvl) {
		case 1:
			return ($GLOBALS["x_login_usr"] == NULL && $GLOBALS["ox_login_usr"] != NULL) ||
				($GLOBALS["x_login_usr"] !== NULL && $GLOBALS["ox_login_usr"] == NULL) ||
				($GLOBALS["x_login_usr"] <> $GLOBALS["ox_login_usr"]);
	}
}
...
function GetRow($opt) {
	...
	while ($row) {
		if (ValidRow($row)) {
			$GLOBALS["x_login_usr"] = $row["login_usr"];
...
function InitReportData(&$rs) {
	...
	foreach ($rs as $row) {
		...
		// Update group count
		if ($bValidRow) {
			$x_login_usr = $row["login_usr"];
			$gx_login_usr = $x_login_usr;
			$bNewGroup = ($grpcnt == 0) ||
				($grpvalue == NULL && $x_login_usr != NULL) ||
				($grpvalue != NULL && $x_login_usr == NULL) ||
				(@$grpvalue <> $gx_login_usr);
			if ($bNewGroup) {
				$grpvalue = $gx_login_usr;
				$grpcnt++;
			}
...
function ProcessChartData(&$rs) {
	AccumulateChartSummary($rs, $GLOBALS["Incidente_por_Tecnico_cht_smry"], $GLOBALS["Incidente_por_Tecnico_cht_XFld"], "", $GLOBALS["Incidente_por_Tecnico_cht_YFld"], 0, "", "SUM");
}
?>
<?php
