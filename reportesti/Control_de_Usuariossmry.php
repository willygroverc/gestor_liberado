...
// PHP Report Maker 1.0 - Table level configuration (Control de Usuarios)
// Table Level Constants

define("EW_TABLE_VAR", "Control_de_Usuarios", TRUE);
define("EW_TABLE_GROUP_PER_PAGE", "grpperpage", TRUE);
define("EW_TABLE_SESSION_GROUP_PER_PAGE", "Control_de_Usuarios_grpperpage", TRUE);
define("EW_TABLE_START_GROUP", "start", TRUE);
define("EW_TABLE_SESSION_START_GROUP", "Control_de_Usuarios_start", TRUE);
define("EW_TABLE_SESSION_SEARCH", "Control_de_Usuarios_search", TRUE);
define("EW_TABLE_CHILD_USER_ID", "childuserid", TRUE);
define("EW_TABLE_SESSION_CHILD_USER_ID", "Control_de_Usuarios_childuserid", TRUE);
...
define("EW_TABLE_SQL_ORDERBY", "control_usr.NombreUsr ASC", TRUE);
...
// Group variables
$x_NombreUsr = NULL; $ox_NombreUsr = NULL; $gx_NombreUsr = NULL; $dgx_NombreUsr = NULL; $tx_NombreUsr = NULL; $ftx_NombreUsr = 200; $gfx_NombreUsr = $ftx_NombreUsr; $gbx_NombreUsr = ""; $gix_NombreUsr = "0"; $rf_NombreUsr = NULL; $rt_NombreUsr = NULL;
...
Control de Usuarios
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="Control_de_Usuariossmry.php?export=html">Imprimir</a>
&nbsp;&nbsp;<a href="Control_de_Usuariossmry.php?export=excel">Exportar a Excel</a>
&nbsp;&nbsp;<a href="Control_de_Usuariossmry.php?export=word">Exportar a Word</a>
<?php } ?>
<br><br>
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php if (@$sExport == "") { ?>
<form action="Control_de_Usuariossmry.php" name="ewpagerform" id="ewpagerform">
...
<!--first page button-->
	...
	<?php } else { ?>
	<td><a href="Control_de_Usuariossmry.php?start=1"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($PrevStart == $nStartGrp) { ?>
	<td><img src="rptimages/prevdisab.gif" alt="Previous" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Control_de_Usuariossmry.php?start=<?php echo $PrevStart; ?>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } ?>
<!--current page number-->
...
<!--next page button-->
	...
	<?php } else { ?>
	<td><a href="Control_de_Usuariossmry.php?start=<?php echo $NextStart; ?>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>
	<?php  } ?>
<!--last page button-->
	<?php if ($LastStart == $nStartGrp) { ?>
	<td><img src="rptimages/lastdisab.gif" alt="Last" width="16" height="16" border="0"></td>
	<?php } else { ?>
	<td><a href="Control_de_Usuariossmry.php?start=<?php echo $LastStart; ?>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>
	<?php } ?>
...
<?php
	...
	if (intval($nGrpCount) >= intval($nStartGrp) && $nGrpCount <= $nStopGrp) {
		...
		// Show group values
		$dgx_NombreUsr = $x_NombreUsr;
		if (($x_NombreUsr == NULL && $ox_NombreUsr == NULL) ||
			(($x_NombreUsr <> "" && $ox_NombreUsr == $dgx_NombreUsr) && !ChkLvlBreak(1))) {
			$dgx_NombreUsr = "";
		} elseif ($x_NombreUsr == NULL) {
			$dgx_NombreUsr = EW_NULL_LABEL;
		} elseif ($x_NombreUsr == "") {
			$dgx_NombreUsr = EW_EMPTY_LABEL;
		}
?>
	<tr<?php echo $sItemRowClass; ?>>
		<td class="ewRptGrpField1">
		<?php $tx_NombreUsr = $x_NombreUsr; $x_NombreUsr = $dgx_NombreUsr; ?>
<?php echo $x_NombreUsr ?>
		<?php $x_NombreUsr = $tx_NombreUsr; ?></td>
...
function ChkLvlBreak($lvl) {
	switch ($lvl) {
		case 1:
			return ($GLOBALS["x_NombreUsr"] == NULL && $GLOBALS["ox_NombreUsr"] != NULL) ||
				($GLOBALS["x_NombreUsr"] !== NULL && $GLOBALS["ox_NombreUsr"] == NULL) ||
				($GLOBALS["x_NombreUsr"] <> $GLOBALS["ox_NombreUsr"]);
	}
}
...
function InitReportData(&$rs) {
	...
	foreach ($rs as $row) {
		...
		// Update group count
		if ($bValidRow) {
			$x_NombreUsr = $row["NombreUsr"];
			$gx_NombreUsr = $x_NombreUsr;
			$bNewGroup = ($grpcnt == 0) ||
				($grpvalue == NULL && $x_NombreUsr != NULL) ||
				($grpvalue != NULL && $x_NombreUsr == NULL) ||
				(@$grpvalue <> $gx_NombreUsr);
			if ($bNewGroup) {
				$grpvalue = $gx_NombreUsr;
				$grpcnt++;
			}
