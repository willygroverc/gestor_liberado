...
if (isset($INSERTAR)){  
	...
	$IdProgTarea1=SanitizeString($IdProgTarea1);
		$sql="UPDATE progtareasmensual1 SET RevisadoPor='$_SESSION[login]', RevisadoPorFecha='".date("Y-m-d H:i:s")."', Aprobacion='$Aprobacion', RevisadoPorObs='$Observaciones', Revisado=1 WHERE IdProgTarea1=$IdProgTarea1";
		mysql_query( $sql);
	}
	else {
		$sql = "SELECT *,  DATE_FORMAT(FechaProceso, '%d/%m/%Y') AS FechaProceso FROM progtareasmensual1 
		...
		{	
			...
			$Aprobacion=SanitizeString($Aprobacion);
			$Observaciones=SanitizeString($Observaciones);
			$sql = "INSERT INTO progtareasmensual1 (IdProgTarea, FechaProceso, RealizadoPor, Realizacion, RealizadoPorObs) VALUES ($IdProgTarea, '".date("Y-m-d H:i:s")."', '$_SESSION[login]', '$Aprobacion', '$Observaciones')";
			mysql_query( $sql);
...
<table width="97%" border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
  <tr>
    <th colspan="8" bgcolor="#006699"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif">PROGRAMACION DE TAREAS - MENSUALES</font></th>
  </tr>
  <tr align="center">
  ...
  <?php
  $IdProgTarea=SanitizeString($IdProgTarea);
		$sql = "SELECT *, DATE_FORMAT(FechaDe, '%d/%m/%Y') AS FechaDe FROM progtareasmensual WHERE IdProgTarea='$IdProgTarea'";
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
  		 ?>
  <tr align="center">
  	<td><?phpecho $row['IdProgTarea'];?></td>
      <td><?phpecho $row['FechaDe'];?></td>
...
if(isset($menu)){
	...
	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM progtareasmensual1 WHERE IdProgTarea='$IdProgTarea' $cond";
	...
	$_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
		...
		FROM progtareasmensual1 WHERE IdProgTarea='$IdProgTarea' $cond ORDER BY IdProgTarea1 DESC LIMIT $_pagi_inicial,$_pagi_cuantos";
		$rs=mysql_query($sql);
		print mysql_error();
		while($tmp=mysql_fetch_array($rs)){
			print "<tr>";
			...
			if($tmp["Revisado"]==1) print "<td align=\"center\"><img src=\"images/ok.gif\" border=\"0\"></td>";
				...
				if($_SESSION['tipo']=="A"  || $tmp['RealizadoPor']!=$_SESSION['login']) print "<td align=\"center\"><a href=\"prog_tareasmensualrev.php?do=revisar&IdProgTarea=$IdProgTarea&IdProgTarea1=$tmp[IdProgTarea1]\"><img src=\"images/no3.gif\" border=\"0\"></a></td>";
				else 
					print "<td align=\"center\"><img src=\"images/no2.gif\" border=\"0\"></td>";				
			}
			print "</tr>";
		}
  		 ?>  
...
<form action="prog_tareasmensualrev.php" method="get" name="form2" id="form2" onKeyPress="return Form()">
	<input name="IdProgTarea" type="hidden" value="<?php echo @$IdProgTarea;?>">
	<input name="IdProgTarea1" type="hidden" id="IdProgTarea1" value="<?php echo @$_GET['IdProgTarea1'];?>">
...
<?php
	...
	$sw = 0;
	$sql = "SELECT *,  DATE_FORMAT(FechaProceso, '%d/%m/%Y') AS FechaProceso FROM progtareasmensual1 WHERE IdProgTarea='$IdProgTarea'";			
	$res = mysql_query($sql);			
	While ( $fila = mysql_fetch_array($res))
	{	$str = date("d/m/Y");			
		if ( $str == $fila['FechaProceso'] and $_SESSION['login']== $fila['RealizadoPor'])
...
function enviar(){
	...
	open("report_tarea.php?IdProgTarea="+IdProgTarea+"&tarea=mensual","TAREAS",'width=590,height=200,status=no,resizable=no,toolbars=no,top=0,left=250,dependent=yes,alwaysRaised=yes')
}
//-->
</script>
