...
$data="<chart caption='Tiempo de Solucion (Porcentual)' shownames='1' showvalues='1' decimals='0' numberPrefix='Ordenes '>";
////desde aqui
//==========ORDENES DE TRABAJO==========================-
$sql="SELECT count(*) as num FROM conformidad WHERE tiemposol_conf = 1 $sql_alt";
$rs = mysql_db_query($db,$sql,$link);
$tmp = mysql_fetch_array($rs);
$data.="<set label='Malo' value='$tmp[num]'/>";

$sql="SELECT count(*) as num FROM conformidad WHERE tiemposol_conf = 2 $sql_alt";
$rs = mysql_db_query($db,$sql,$link);
$tmp = mysql_fetch_array($rs);
$data.="<set label='Bueno' value='$tmp[num]'/>";

$sql="SELECT count(*) as num FROM conformidad WHERE tiemposol_conf = 3 $sql_alt";
$rs = mysql_db_query($db,$sql,$link);
$tmp = mysql_fetch_array($rs);
