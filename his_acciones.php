...
?>"><!--BORRAR ESTE ENLACE!!!--></div></td>
    ...
    <?php
	/*$sw=0;
	$sql_v = "SELECT user_a FROM transferencia WHERE user_a='$num'";
	...
	while($row_v=mysql_fetch_array($result_v)) 
  	{
		...
		$sql = "Select * from acciones_bk where id_accionistas='$num'";
		//echo $sql;
		...
		//*******2******************************************************************++
		$sql = "Select * from transferencia where user_de = '$num' OR user_a='$num'";	
		//echo "<br>";
		//echo $sql;
		$result=mysql_query($sql);
		while($row=mysql_fetch_array($result)) 
  		{
		$sql_tr="SELECT * FROM `acciones` WHERE id_acciones = '$row[id_acc]'";
		//echo "<br>"."<br>";
		//echo $sql_tr;
		...
		<tr align="center"> 
		  <td>&nbsp;<?php 
			echo $row_tr[id_accionistas];
		  ?></td>
		  <td>&nbsp;<?php 
		  echo $row_tr[serie_ac];
		  ?></td>
		  <td>&nbsp;<?php 
		  echo $row_tr[valor_ac];
		  ?></td>
		  <td>&nbsp;<?php 
		  echo $row_tr[fecas_ac];
		  ?></td>
		  <td>&nbsp;<?php echo number_format($row_tr[num_ac],0,'.',',');
		  ?></td>
		  <?php
		  $sb_total=$row_tr[valor_ac]*$row_tr[num_ac];
		  ?>
		  <td>&nbsp;<?php 
		  echo number_format($sb_total,2,'.',',');
		  ?></td>
		  <td>&nbsp;<?php 
			echo $row_tr[class_ac];
		  ?></td>
		  <?php
		    $sql_us="SELECT * FROM `transferencia` WHERE id_acc = '$row[id_acciones]'";
		  //echo "<br>";
		...
		//echo "<script>alert('$row[id_acciones]');</script>";
		  ...
		  <td>
		   ...
		   if($row[user_de])
		   {
			$sql_nom="SELECT nom_acc FROM `accionistas` WHERE id_acc = '$row[user_de]'";
		  ...
		  <td>
		   ...
		   if($row[user_a])
		   {
			$sql_nom="SELECT nom_acc FROM `accionistas` WHERE id_acc = '$row[user_a]'";
			$res_nom=mysql_query($sql_nom);
			$row_nom=mysql_fetch_array($res_nom);
			echo $row_nom[nom_acc];
		   }
