...
$dir="C:/SARC/Soluciones";
$path="/SR".$AE."0".$ME.$DE."S.IFC";
$ext="";
//echo date("m");
//echo date("Y");
$fecha = date("d-m-Y"); $separar = explode('-',$fecha);
if(($ME == '01') OR ($ME == '02') OR ($ME == '03') OR ($ME == '04') OR ($ME == '05') OR ($ME == '06') OR ($ME == '07') OR ($ME == '08') OR ($ME == '09')) {
	$sqlfr="UPDATE solucion_reclamos SET DFechaReporte='".$AE."-"."0".$ME."-".$DE."' WHERE TFechaSolucion BETWEEN '".$AA."-"."0".$MA."-"."0".$DA."' and '".$AE."-"."0".$ME."-".$DE."'";
	$resultfr=mysql_db_query($db,$sqlfr,$link);
	$sqlexp="SELECT CTipoEntidad, CCorrelativoEntidad,CReclamo, TGestion, TFechaSolucion, TCiteRespuesta, TGlosaRespuesta, TResultado, DFechaReporte INTO OUTFILE '".$dir.$path.$ext."' FIELDS TERMINATED BY '|' FROM solucion_reclamos WHERE TFechaSolucion BETWEEN '".$AA."-"."0".$MA."-"."0".$DA."' and '".$AE."-"."0".$ME."-".$DE."' order by CReclamo ASC";
	echo "<script>alert('AlertA')</script>";
}
elseif(($ME == '10') OR ($ME == '11') OR ($ME == '12'))
{
	$sqlfr="UPDATE solucion_reclamos SET DFechaReporte='".$AE."-"."0".$ME."-".$DE."' WHERE TFechaSolucion BETWEEN '".$AA."-".$MA."-".$DA."' and '".$AE."-".$ME."-".$DE."'";
	$resultfr=mysql_db_query($db,$sqlfr,$link);
	$sqlexp="SELECT CTipoEntidad, CCorrelativoEntidad,CReclamo, TGestion, TFechaSolucion, TCiteRespuesta, TGlosaRespuesta, TResultado, DFechaReporte INTO OUTFILE '".$dir.$path.$ext."' FIELDS TERMINATED BY '|' FROM solucion_reclamos WHERE TFechaSolucion BETWEEN '".$AA."-".$MA."-".$DA."' and '".$AE."-".$ME."-".$DE."' order by CReclamo ASC";
	echo "<script>alert('$sqlexp')</script>";
}
...
<tr>
  <td  background="images/main-button-tileR1.jpg" align="center"><font color="#FFFFFF" size="2" face="Arial, Helvetica"><b>EXPORTAR SOLUCION</b></font></td>
</tr>
