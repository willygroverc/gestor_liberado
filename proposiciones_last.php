...
if ($adjuntar)
{
		...
		if($archivo_size < $tam_max)
		{
			...
			header("location: proposiciones_last.php?id_minuta=$id_minuta&prop=$prop&nombre=$nombre");
			
		}else{
			unset($msg);
			$msg="EL TAMAÑO DEL ARCHIVO ADJUNTO NO DEBE SER MAYOR A ".$row5[tam_archivo]." Mb";
			header("location: proposiciones_last.php?id_minuta=$id_minuta&msg=$msg&prop=$prop&nombre=$nombre");
		}
}
...
if ($Terminar){ 
	...
	header("location: minuta_last.php?cad=$cad&id_minuta=$id_minuta&verif=$verif&insertado=$insertado");
}
if ($reg_form)
{	
	session_start();
	...
	if($adjuntos_bo){ 
		$sql1="UPDATE asistentes SET prop='$prop', adjunto= '".$adjuntos_bo."', hash_archivo = '".$adjuntos_bo_hash."' WHERE 	
		nombre='$nombre' AND id_minuta='$id_minuta'";
	...
	$_SESSION[$var] = NULL;
	$_SESSION[$var_hash] = NULL;
	header("location: proposiciones_last.php?id_minuta=$id_minuta&verif=2&dato=$dato&insertado=$insertado");
}
include("top.php");
...
<table width="58%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#006699"  background="images/fondo.jpg" bgcolor="#EAEAEA">
  <form name="form2" method="post" action="<?php echo $PHP_SELF ?>" onKeyPress="return Form()" enctype="multipart/form-data">
	<input name="id_minuta" type="hidden" value="<?php echo $id_minuta;?>">
	<input name="idmin" type="hidden" value="<?php=$idmin?>">
      ...
      <td > 
        <table width="100%" border="2" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
          ...
          <tr> 
			...
			<th width="45%" nowrap bgcolor="#006699"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif">ARCHIVO 
              ADJUNTO </font></th>
          </tr>
          <?php
		$cont=0;	
		...
		while($row=mysql_fetch_array($result)) 
  		{
			  ...
			  <tr> 
				...
				<td align="center">
				  ...
				  <?php
				  		if($row[adjunto] <> "") 
						{	echo "<tr><td align = center><a href='adjuntosa.php?id_minuta=$id_minuta&nom=".$row[nombre]."&cont=$cont&pag=proplast'>VER	
							ARCHIVOS</a></td></tr>";
						}
				  ?>
				  </table>
		 ...
		 ?>
          ...
          <tr> 
            ...
            <td width="198" nowrap height="7"><div align="center"><strong>
                <textarea name="prop" cols="40" rows="4" id="prop"><?php=$prop?></textarea>
                </strong> </div></td>
			<td align="center">
				<table>
				<!---------------------------------------------------------------------------------------------->
