...
{	
	...
	{	$fecha_r=SanitizeString($fecha_r);
		...
		$fecha_r="$Ano-$Mes-$Dia";
		$sql6="UPDATE cambios_prueba SET fecha_pru='$fecha_r',obj_pru='$obj_pru',obs_pru0='$obs_pru0',obs_pru1='$obs_pru1' ".
		"WHERE id_orden='$orden' AND tipo_pru='$tpo'";
		mysql_query($sql6);
		
		$sql_i = "SELECT id_pru FROM cambios_prueba WHERE id_orden='$orden' AND tipo_pru='$tpo'";
    ...
    <tr> 
      <th>PRUEBAS 
        <?php	$tp2=md5($orden);
			...
			{	echo "USUARIAS"; 
				$tpo="1";
			 	$sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba WHERE id_orden='$orden' AND tipo_pru='1'";
			}
			elseif($tp=="7d428547b8723786029735cde7b75d7d_".$tp2)
			{
				echo "DE SISTEMAS"; 
				$tpo="2";
				$sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba WHERE id_orden='$orden' AND tipo_pru='2'";
			}
			elseif($tp=="f4b29b15eee509a8fa39344471561d29_".$tp2)
			{
				echo "DE SEGURIDAD"; 
				$tpo="3";
				$sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba WHERE id_orden='$orden' AND tipo_pru='3'";
			}
