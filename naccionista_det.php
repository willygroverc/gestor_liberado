...
{	
		...
		{
			$sql="UPDATE acciones SET serie_ac='$serie_ac', valor_ac='$valor_ac', fecas_ac='$fecas_ac',num_ac='$num_ac', class_ac='$class_ac' WHERE id_acciones=".$_SESSION['test']."";
		}
...
?>">Modificar</div></td>
  ...
  <table width="75%" border="2" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
	...
	?>
    <tr align="center"> 
      <td>&nbsp;<?php echo "<a href=\"naccionista_det.php?np=$row[id_acciones]&num=$num\">$cont</a>";?></td>
	...
	if(!empty($np))
	{		
		$sql_ed = "SELECT * FROM acciones WHERE id_acciones='$np'";
    ...
    <tr> 
      <td height="7" nowrap bgcolor="#006699">
	  <?php
	  $cuenta_id=1;
	  $sql="SELECT * FROM acciones WHERE id_accionistas='$num' ORDER BY id_ac ASC";
	  $recordset=mysql_query($sql);
		echo '<select name="selecciona" onChange="cambia(this.value)">';
		echo '<option value="nuevo" selected>Nuevo</option>';
		for ($i=1;$i<=mysql_num_rows($recordset);$i++){
			$fila_acc=mysql_fetch_array($recordset);
			if ($fila_acc['id_acciones']==$np)
				echo '<option value="'.$fila_acc['id_acciones'].'" selected>'.$cuenta_id.'</option>';
			else
				echo '<option value="'.$fila_acc['id_acciones'].'">'.$cuenta_id.'</option>';
			$cuenta_id++;
			}
