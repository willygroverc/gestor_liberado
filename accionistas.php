...
if (isset($traspaso))	{ 
		    header("location: naccionista_tr.php");
}
if (isset($NAccionista))	{ 
		    header("location: naccionista.php");
}
...
<table width="75%" border="0" cellpadding="0" cellspacing="0" background="images/fondo.jpg">
  <td height="68" valign="top"><table width="100%" border="1" align="center" cellpadding="0" cellspacing="2">
        ...
        <tr>
		  ...
		  <th class="menu" height="20" background="images/main-button-tileR1.jpg">HISTORICO</th>
		  <th class="menu" height="20" background="images/main-button-tileR1.jpg">GRAVAMENES</th>
...
if(!isset($ord_tab)) $ord_tab='id_acc';
	...
	//$sql = "SELECT DISTINCT a.id_acc, a.nom_acc, a.fecha_acc, a.nac_acc, a.dom_acc, a.tel_acc, a.fec_acc, a.estado, SUM( b.num_ac ) AS num, SUM( b.valor_ac ) AS mont FROM `accionistas` AS a, acciones AS b WHERE a.id_acc = b.id_accionistas ORDER BY $ord_tab $ali LIMIT $_pagi_inicial,$_pagi_cuantos";
	$sql_v= "select distinct b.id_accionistas from accionistas AS a,acciones AS b WHERE a.id_acc = b.id_accionistas";
	$result_v=mysql_query($sql_v);
		while($fila=mysql_fetch_array($result_v))
		{
			$sql="SELECT a.*, DATE_FORMAT(fecha_acc, '%d/%m/%Y') AS fecha_acc, SUM(b.num_ac) AS num, SUM(b.valor_ac) AS mont FROM accionistas a, acciones b WHERE a.id_acc='$fila[id_accionistas]' AND a.id_acc = b.id_accionistas ORDER BY $ord_tab $ali LIMIT $_pagi_inicial,$_pagi_cuantos";
			...
			/*	$sql_acc="SELECT SUM(num_ac) AS num, SUM(valor_ac) AS mont FROM acciones WHERE id_acc='$row[id_acc]'";
				...
				echo "<td><a href=\"gravamen.php?num=".$row['id_acc']."\"><img src=\"images/gravamen_list.png\" border=\"0\" alt=\"ANTERIOR\"></a></td>";
...
<form action="" method="post">
  <div align="center">
    ...
    &nbsp;&nbsp;&nbsp; 
    <input type="submit" name="sin_acciones" value="PERSONAS SIN ACCIONES REGISTRADAS">
  </div>
</form> 
