...
{   
	if($action=="revisar"){
		$sql="UPDATE progtareasanual1 SET RevisadoPor='$_SESSION[login]', RevisadoPorFecha='".date("Y-m-d H:i:s")."', Aprobacion='$Aprobacion', RevisadoPorObs='$Observaciones', Revisado=1 WHERE IdProgTarea1=$IdProgTarea1";
		mysql_db_query($db, $sql);
	}
	else {
		$sql="INSERT INTO progtareasanual1 (IdProgTarea, FechaProceso, RealizadoPor, Realizacion, RealizadoPorObs) VALUES ($IdProgTarea, '".date("Y-m-d H:i:s")."', '$_SESSION[login]', '$Aprobacion', '$Observaciones')";
		mysql_db_query($db, $sql);
	}
...
<table width="95%" border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
  <tr>
    <th colspan="7" bgcolor="#006699"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif">PROGRAMACION DE TAREAS - ANUALES</font></th>
  </tr>
  <tr align="center">
  ...
  <?php
		$sql = "SELECT *, DATE_FORMAT(FechaDe, '%d/%m/%Y') AS FechaDe, DATE_FORMAT(Dia, '%d/%m/%Y') AS Dia FROM progtareasanual WHERE IdProgTarea='$IdProgTarea'";
  ...
  <tr align="center">
      ...
      <td><div align="center"><?php=$row["Dia"]?></div></td>	  
...
default:
	...
	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM progtareasanual1 WHERE IdProgTarea='$IdProgTarea' $cond";
	...
	$_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
		...
		FROM progtareasanual1 WHERE IdProgTarea='$IdProgTarea' $cond ORDER BY IdProgTarea1 DESC LIMIT $_pagi_inicial,$_pagi_cuantos";
		...
		while($tmp=mysql_fetch_array($rs)){
			...
			if($tmp["Revisado"]==1) print "<td align=\"center\"><img src=\"images/ok.gif\" border=\"0\"></td>";
				...
				if($_SESSION[tipo]=="A"  || $tmp[RealizadoPor]!=$_SESSION[login]) print "<td align=\"center\"><a href=\"prog_tareasanualrev.php?do=revisar&IdProgTarea=$IdProgTarea&IdProgTarea1=$tmp[IdProgTarea1]\"><img src=\"images/no3.gif\" border=\"0\"></a></td>";
...
<form action="prog_tareasanualrev.php" method="get" name="form2" id="form2" onKeyPress="return Form()">
...
function enviar(){
	...
	open("report_tarea.php?IdProgTarea="+IdProgTarea+"&tarea=anual","TAREAS",'width=590,height=200,status=no,resizable=no,toolbars=no,top=0,left=250,dependent=yes,alwaysRaised=yes')
}
//-->
</script>
