...
{	
	...
	if($tp_e[1]==$ha)
	{
		$sql_i = "SELECT MAX(num_pru) AS num_pru FROM cambios_prueba2_planif WHERE id_pru=$num";
		...
		{	require_once('funciones.php');
			...
			$sql="INSERT INTO cambios_prueba2_planif (id_pru,num_pru,desc_pru,tiem_est,obs_pru)".
			"VALUES ('$num','$num_pru1','$desc_pru','$tiem_est','$obs_pru')";
		}
		else
		{	require_once('funciones.php');
			$num=SanitizeString($num);
			...
			$tiem_est=SanitizeString($tiem_est);
			$obs_pru=SanitizeString($obs_pru);
			$sql="UPDATE cambios_prueba2_planif SET desc_pru='$desc_pru',tiem_est='$tiem_est',".
			"obs_pru='$obs_pru' WHERE id_pru='$num' AND num_pru='$selecciona'";
		}
		mysql_db_query($db,$sql,$link);
	}
	else
	{ header("location: lista_prueba_planif.php?Naveg=Cambios >> Pruebas&msg=2");}												
}
if (isset($Terminar))
{
	header("location: lista_prueba_planif.php?Naveg=Cambios >> Pruebas");
}

include("top.php");
if(!empty($_GET['id_minuta'])) $id_minuta=($_GET['id_minuta']);
...
?>  
  ...
  <table width="95%" border="2" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
    ...
    <tr> 
      <th width="90" nowrap bgcolor="#006699"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">N&ordf;</font></th>
      <th width="321" nowrap bgcolor="#006699"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">DESCRIPCION 
        DE LA PRUEBA</font></th>
      <th width="185" nowrap bgcolor="#006699"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">TIEMPO 
        ESTIMADO (Minutos)</font></th>
      <th width="303" nowrap bgcolor="#006699"><font size="1" face="Arial, Helvetica, sans-serif" color="#FFFFFF">OBSERVACIONES</font></th>
    </tr>
    <?php
	...
	?>
    <tr align="center"> 
      <td>&nbsp;<?php echo "<a href=\"cambio_pru2_last_planif.php?orden=$orden&tp=$tp&np=$row[num_pru]&num=$num\">$row[num_pru]</a>";?></td>
	...
	?>
    <tr> 
      <td width="90" height="7" nowrap><div align="center">
      ...
      <td nowrap><div align="center"><strong> 
          <textarea name="desc_pru" cols="40"><?php if(!empty($row_ed['desc_pru'])) echo $row_ed['desc_pru'];?></textarea>
          </strong></div></td>
      <td nowrap><div align="center"><strong> 
          <input name="tiem_est" type="text" size="15" maxlength="15" value="<?php if(!empty($row_ed['tiem_est'])) echo $row_ed['tiem_est'];?>">
          Min. </strong></div></td>
       <td width="303" height="7" nowrap><div align="center"><strong> 
          <textarea name="obs_pru" cols="40"><?php if(!empty($row_ed['obs_pru'])) echo $row_ed['obs_pru'];?></textarea>
          </strong></div></td>
    </tr>
	<tr> 
...
{        
		...
		if (numero == "nuevo") {irapagina_c("cambio_pru2_last_planif.php?orden="+orden+"&tp="+tp+"&num="+num);}
		else {irapagina_c("cambio_pru2_last_planif.php?orden="+orden+"&tp="+tp+"&np="+numero+"&num="+num);}
}
