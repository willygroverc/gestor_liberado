    ...
    <td height="68" valign="top"><table width="100%" border="1" align="center" cellpadding="4" cellspacing="0">
		...
		switch ($menu) {
		   	case "GENERAL":	
				...
				break;
			case "TECNICO":
				$sql="SELECT ordenes.*, DATE_FORMAT(fecha, '%d/%m/%Y') AS fecha FROM ordenes WHERE cod_usr='$nombre' AND $condicion2 ORDER BY id_orden DESC ";
				break;
			case "AREA":
				...
				WHERE ordenes.cod_usr=users.login_usr AND users.area_usr='$nombre' AND $condicion2 ORDER BY id_orden DESC ";				
				break;
			case "ASIGNADO":			
				$condicion="users.login_usr='$nombre'";
				$condicion3 = "fecha_asig BETWEEN '$fec1' AND '$fec2'";					
				
				$sql = "SELECT DISTINCT(id_orden), MAX(id_asig) FROM asignacion WHERE asig='$nombre' AND $condicion3 GROUP BY id_orden";
			...
			case "CLIENTE":
				...
				WHERE ordenes.cod_usr= users.login_usr AND users.login_usr='$nombre' AND users.tipo2_usr='C' AND $condicion2 ORDER BY id_orden DESC ";				
				break;
			case "CIUDAD":
				...
				WHERE ordenes.cod_usr=users.login_usr AND users.ciu_usr ='$nombre'  AND $condicion2 ORDER BY id_orden DESC";				
				break;
			case "ADICIONAL1":
