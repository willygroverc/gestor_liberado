<?php if (isset($TERMINAR)) header("location: lista_mantenimiento.php");
include ("top.php");
if (isset($GUARDAR)){
  include("conexion.php");
    ...
    if($edit != 1)
	{	$sql1 = "INSERT INTO cronograma (TareCrono,FeProIni,FeProTer,FeRealIni,FeRealTer,RubricaR,Observ,OrdAyuda) ".
		"VALUES ('$TareCrono','$FeProIni','$FeProTer','$FeRealIni','$FeRealTer','$RubricaR','$Observ','$var')";
	}else
	{  $sql1 = "UPDATE cronograma SET TareCrono = '$TareCrono', FeProIni = '$FeProIni' , FeProTer = '$FeProTer',".
		" FeRealIni = '$FeRealIni', FeRealTer = '$FeRealTer', RubricaR = '$RubricaR', Observ = '$Observ' , OrdAyuda = '$var' ".
		"WHERE IdCrono = $id";
    }	
...
<table border="1" align="center" cellpadding="0" cellspacing="0" background="images/fondo.jpg">
  ...
  <tr> 
      <td width="9%" height="26" bgcolor="#006699"> 
        <div align="center"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong>Inicio</strong></font></div></td>
...
//modificado desde aqui------------>
		...
		?>
  <tr align="center"> 
      <td nowrap>&nbsp;<a href="llenadoUS_2_last.php?var=<?php=$var?>&fase=<?php=$row[TareCrono]?>&num=<?php=$c?>&edit=1&id=<?php=$row[IdCrono]?>"><?php echo $c;?></a></td>
    <td nowrap>&nbsp; 
      <?php=$row[TareCrono]?>
    </td>
    <td nowrap>&nbsp; 
...
<table border="1" align="center" cellpadding="0" cellspacing="0" background="images/fondo.jpg">
  <tr> 
    <?php if($edit == 1){?>
	<td colspan="9" bgcolor="#006699"> <div align="center"><font color="#FFFFFF" size="3"><strong>MODIFICAR</strong></font></div></td>
	<?php }else{?>
  ...
  <tr align="center"> 
    <td nowrap>&nbsp;<?php if(!isset($fase))echo "Nuevo";
						else echo "Modificar";
	?></td>
    <?php 
	...
	if(isset($fase)){
		$sql_mod="SELECT * FROM cronograma WHERE OrdAyuda='$var' AND TareCrono='$fase'";
	...
	if(!isset($fase)){
		for($i=1;$i<13;$i++){
			...
			echo "<option value=\"$fase\">$num $fase</option>";
		}
	?>
      </select> </td>
    <td nowrap>&nbsp; <select name="dia1" id="select14">
        <?php		  		
				if(!isset($fase))$fsist=date("Y-m-d");
				else $fsist=$row_mod['FeProIni'];
...
//-->
  ...
  <tr> 
        ...
        <select name="RubricaR" id="RubricaR">
          ...
          <?php 
			  ...
			  while ($row2 = mysql_fetch_array($result2)) 
				{
				if ($row_mod[RubricaR]==$row2[login_usr])
    ...
    <td colspan="3"> <div align="center"> 
        <textarea name="Observ" cols="80" id="Observ"><?php echo $row_mod[Observ];?></textarea>
      </div></td>
  </tr>
</table>
<p align="center"> 
  ...
  <?php }else{?>
  <input name="GUARDAR" type="submit" id="GUARDAR" value="INSERTAR" <?php print $valid->onSubmit() ?>>
  <?php }?>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
