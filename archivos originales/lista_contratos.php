...
if ($RETORNAR){echo "<script type=\"text/javascript\">
           history.go(-2);
       </script>";}
if ($Resumen){ header("location: contratos_resumen.php");}
if ($Contrato){ header("location: contrato1.php");}
include ("top.php");
$IdContra=($_GET['IdContra']);
$Ejecuc=($_GET['Ejec']);
$Cierre=($_GET['Cierre']);
if ($Ejecuc<>"")
{	$sql3="UPDATE contratodatos SET Ejecucion='$Ejecuc' WHERE IdContra='$IdContra'";
	mysql_db_query($db,$sql3,$link);
}
if ($Cierre<>"")
{	$sql3="UPDATE contratodatos SET Cierre='$Cierre' WHERE IdContra='$IdContra'";
	mysql_db_query($db,$sql3,$link);
}
if ($Cierre==0){
