<?php
// Objetivo:	Optimizaci�n de consulta SQL, para listado de ordenes de trabajo
//				Modificaci�n de metodo de envio de datos (a POST)
...
$fechahoy = date('Y-m-d');
if ($_SESSION['tipo']=='A'){
	$sql_ordenes="SELECT DISTINCT(a.asig), o.fecha, o.time, o.cod_usr, o.desc_inc, o.tipo, o.nomb_archivo, o.area, o.dominio, o.objetivo, o.ci_ruc, o.id_anidacion, o.origen, o.hash_archivo, o.observaciones,
			 o.id_orden, a.fechaestsol_asig, s.login_sol, CONCAT(u1.nom_usr, ' ',u1.apa_usr, ' ',u1.ama_usr) as nombre, u1.tipo2_usr, CONCAT(u2.nom_usr, ' ',u2.apa_usr, ' ',u2.ama_usr) as nombre_asig, c.reg_conf
...
$sql_ordenes="SELECT DISTINCT (o.id_orden), o.fecha, o.time, o.cod_usr, o.desc_inc, o.tipo, o.nomb_archivo, o.area, o.dominio, o.objetivo, o.ci_ruc, o.id_anidacion, o.origen, o.hash_archivo, o.observaciones,
				...
				CONCAT(u2.nom_usr, ' ',u2.apa_usr, ' ',u2.ama_usr) as nombre_asig, c.reg_conf
			FROM ordenes o LEFT JOIN asignacion a ON o.id_orden=a.id_orden
			LEFT JOIN solucion s ON o.id_orden=s.id_orden
			LEFT JOIN users u1 ON o.cod_usr=u1.login_usr
			LEFT JOIN users u2 ON a.asig=u2.login_usr
			...
			ORDER BY o.id_orden ASC";
//print_r($sql_ordenes);exit;
}


$recordset_ordenes=mysql_query($sql_ordenes) or die("Error en consulta <br>MySQL dice: ".mysql_error());
...
if (mysql_num_rows($recordset_ordenes)!=0){
	...
	for ($i=1;$i<=mysql_num_rows($recordset_ordenes);$i++){
		...
		if ($i==1 || $fila_ordenes['id_orden']!=$id_aux){
			$id_aux=$fila_ordenes['id_orden'];
			echo '<tr>';
			...
			echo '<td align="center"><img src="images/eliminar.gif" width="16" height="16"></img></td>';
...
echo '  <table width="70%" border="1" align="center">
    ...
    if ($_SESSION['tipo']=="A" or $_SESSION['tipo']=="B" or $_SESSION['tipo']=="S") {
		...
		echo '<input name="estadisticas" type="button" id="estadisticas" value="ESTADISTICAS" onClick="openStat_2();">';
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
