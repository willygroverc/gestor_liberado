...
if (isset($GUARDATOS)) 
{
  ...
  if (ereg('^([0-9a-zA-Z_ -]{1,255})+$',$nombre_mod)) {
	  ...
	  if ($row11['nombre_mod']==$nombre_mod && $row11['desc_mod']==$desc_mod){
		...
		$id_modu=SanitizeString($id_modu);
		...
		$hora_creacion=SanitizeString($hora_creacion);
		$nombre_usr=SanitizeString($nombre_usr);
		$$row11['nombre_mod']=SanitizeString($$row11['nombre_mod']);
