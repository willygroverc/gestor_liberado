...
// Objetivo:	Implementar funciones de exportación a formato PDF
// Fecha:		05/OCT/2012 

// Objetivo1:	Control acceso directo No autorizado.
...
$sql="SELECT login_usr, CONCAT(apa_usr,' ',ama_usr,' ',nom_usr) AS nombre FROM users WHERE tipo2_usr='T' OR tipo2_usr='A' ORDER BY apa_usr ASC";
$rs=mysql_query($sql);
while ($tmp=mysql_fetch_array($rs)) {
	$lstasig[$tmp['login_usr']]=$tmp['nombre'];
}
...
function cambio(numero){        
		 if (!foco_texto){
				 irapagina("orden_estadistical.php?op="+numero);
		 } 
}
var foco_texto=false;
-->
</script>
<script language="javascript" src="js/exporta_reportes.js"></script>
...
<table background="images/fondo.jpg" width="98%"  align="center" border="0">
	<tr>
		<td  bgcolor="#006699" align="center" colspan="3"><font color="#FFFFFF" size="2" face="Arial, Helvetica"><b>ESTADISTICAS</b></font></td>
	<tr>
		<td align="center" colspan="3"><input type="radio" name="opcion" value="G" onClick="cambio(this.value)" <?php if ($op == G) print "checked"; else print "checked"; ?>>
	...
	<tr>
			 ...
			 <?php 
				 ...
				 if ($op != "F"){?>
					<tr>
						<td colspan="2" align="center"><br><input name="IMPRE" type="button" value="    VER    " onClick="OpenPrint()">
						<input name="PDF" type="button" value="EXPORTAR PDF" onClick="pdf_estadisticas()"></td>
					</tr>
				<?php }
				}
				?>					
			...
			<tr>
				...
				<input name="PDF" type="button" value="EXPORTAR PDF" onClick="pdf_estadisticas_fechas()"></td>
				</td>
			</tr>
			...
			function OpenPrint7 () {	
				...
				if (form.menu.value=="GENERAL") window.open ("report_ordenesf.php?DA=" + form.DA.value + "&MA=" + form.MA.value + "&AA=" + form.AA.value + "&DE=" + form.DE.value + "&ME=" + form.ME.value + "&AE=" + form.AE.value + "&menu=" + form.menu.value + "","ESTADICAS",'width=590,height=655,status=no,resizable=no,top=0,left=250,dependent=yes,alwaysRaised=yes,Scrollbars=yes');
...
temp.options[0].selected=true
	...
	function OpenPrint () {
		var form=document.form2;
		if (form.menu.value=="GENERAL") window.open ( "report_ordeneslg.php", "Estadisticas", 'width=590,height=710,status=no,resizable=no,top=0,left=250,dependent=yes,alwaysRaised=yes,Scrollbars=yes' );
		else window.open ( "report_ordenes2.php?menu=" + form.menu.value + "&nombre=" + form.nombre.value + "", "Estadisticas", 'width=590,height=670,status=no,resizable=no,top=0,left=250,dependent=yes,alwaysRaised=yes,Scrollbars=yes' );
		close();
		return false;	
	}
-->
