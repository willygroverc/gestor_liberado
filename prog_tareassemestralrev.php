...
{   
	if(isset($action) && $action=="revisar"){
		$sql="UPDATE progtareassemestral1 SET RevisadoPor='$_SESSION[login]', RevisadoPorFecha='".date("Y-m-d H:i:s")."', Aprobacion='$Aprobacion', RevisadoPorObs='$Observaciones', Revisado=1 WHERE IdProgTarea1=$IdProgTarea1";
		mysql_query( $sql);
	}
	else {
		$sql="INSERT INTO progtareassemestral1 (IdProgTarea, FechaProceso, RealizadoPor, Realizacion, RealizadoPorObs) VALUES ($IdProgTarea, '".date("Y-m-d H:i:s")."', '$_SESSION[login]', '$Aprobacion', '$Observaciones')";
		mysql_query( $sql);
	}
...
<table width="95%" border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
  <tr>
    <th colspan="7" bgcolor="#006699"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif">PROGRAMACION DE TAREAS - SEMESTRALES</font></th>
  </tr>
  <tr align="center">
    ...
    <th width="260" rowspan="2" bgcolor="#006699"><font size="2" face="Arial, Helvetica, sans-serif" color="#FFFFFF">Mes / Dia</font></th>
  ...
  <?php
		$sql = "SELECT *, DATE_FORMAT(FechaDe, '%d/%m/%Y') AS FechaDe FROM progtareassemestral WHERE IdProgTarea='$IdProgTarea'";
  ...
  <tr align="center">
      ...
      <td><div align="center">Mes&nbsp;<?php echo $row["Mes"]?> / Dia <?php=$row["Dia"]?></div></td>	  
...
default:
	...
	$_pagi_sqlConta = "SELECT count(*) AS pagi_totalReg FROM progtareassemestral1 WHERE IdProgTarea='$IdProgTarea' $cond";
	...
	$_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
		...
		$sql = "SELECT *, DATE_FORMAT(FechaProceso, '%d/%m/%Y %H:%i:%s') AS FechaProceso, DATE_FORMAT(RevisadoPorFecha, '%d/%m/%Y %H:%i:%s') AS RevisadoPorFecha 
				FROM progtareassemestral1 WHERE IdProgTarea='$IdProgTarea' $cond ORDER BY IdProgTarea1 DESC LIMIT $_pagi_inicial,$_pagi_cuantos";
		$rs=mysql_query($sql);
		print mysql_error();
		while($tmp=mysql_fetch_array($rs)){
			print "<tr>";
			...
			if($tmp["Revisado"]==1) print "<td align=\"center\"><img src=\"images/ok.gif\" border=\"0\"></td>";
				...
				if($_SESSION[tipo]=="A"  || $tmp[RealizadoPor]!=$_SESSION[login]) print "<td align=\"center\"><a href=\"prog_tareassemestralrev.php?do=revisar&IdProgTarea=$IdProgTarea&IdProgTarea1=$tmp[IdProgTarea1]\"><img src=\"images/no3.gif\" border=\"0\"></a></td>";
...
<form action="prog_tareassemestralrev.php" method="get" name="form2" id="form2" onKeyPress="return Form()">
...
function enviar(){
	...
	open("report_tarea.php?IdProgTarea="+IdProgTarea+"&tarea=semestral","TAREAS",'width=590,height=200,status=no,resizable=no,toolbars=no,top=0,left=250,dependent=yes,alwaysRaised=yes')
}
//-->
</script>
