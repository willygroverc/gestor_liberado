...
<form action="" name="form1" method="get">
  <table width="75%" border="1" background="images/fondo.jpg">
    <tr> 
      <th colspan="4" background="images/main-button-tileR1.jpg"><div align="center"><strong><font color="#FFFFFF">IMPRESION</font></strong></div></th>
    </tr>
	<tr>
		<td colspan="4">&nbsp;</td></tr>
    <tr> 
	  ...
	  <select name="txtarea" id="txtarea" onChange="cambiar(this.value)">
			...
			<?php while($area=mysql_fetch_array($datos)){?>
                        <option value="<?php=$area['area_cod'];?>" <?php if(isset($_REQUEST['id_area'])==$area['area_cod']) echo "selected"?>><?php echo $area['area_nombre']; ?></option>
			<?php }?>
      </select></td>
...
function imprimir(){
	...
	{
		open("impresion_niveles.php")
		close();
	}
	else{
		open("imprimir_objetivo.php?cod1="+document.form1.txtarea.value+"&cod="+document.form1.dominio.value)
		close();
	}
}
