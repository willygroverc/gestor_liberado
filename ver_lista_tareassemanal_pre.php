...
<!--
	...
	{	
		...
		window.open ( "ver_lista_tareassemanal_1.php?mes0=" + mes2 + "&ano0=" + ano + "&tareas=" + tareas + "&selec=" + selec, "Semanal");
	}
	if(selec=="Asignado")
	{	
		var asignado="<?php echo $tipo_asignado?>";
		window.open ( "ver_lista_tareassemanal_1.php?mes0=" + mes2 + "&ano0=" + ano + "&asignado=" + asignado + "&selec=" + selec, "Semanal");
	}
	close();
-->
</script>
...
<table width="98%"  border="1" align="center" cellpadding="2" cellspacing="0" background="images/fondo.jpg">
    <tr> 
      <th background="images/main-button-tileR2.jpg"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif">&nbsp; 
       <?php echo "REPORTE PROGRAMACION DE TAREAS SEMANALES";?>
        </font></th>
    </tr>
    <tr align="center"> 
      <td height="235"> <table width="100%" border="0">
          ...
          <tr> 
            ...
            <td valign="top"><strong><font face="Arial, Helvetica, sans-serif">
              <?php if((isset($select) && $selec=="Tarea") OR !isset($selec)){echo "Tareas:";}else{echo "Asignado a:";}?>
              </font></strong></td>
            <td colspan="2">&nbsp; 
              <?php if(isset($selec) && $selec=="Tarea" OR !isset($selec)){?>
              <select name="tareas[]" size="5" multiple style="width:250px">
                <?php	$sql="SELECT Actividad, IdProgTarea FROM progtareassemanal ORDER BY Dia ASC, HoraDe ASC";	
			...
			elseif($selec=="Asignado"){?>
              <select name="asignado[]" size="5" multiple style="width:250px">
                <?php	
					$sql="SELECT s_asig FROM progtareassemanal WHERE s_asig<>'0' GROUP BY s_asig ORDER BY IdProgTarea ASC";	
					$result=mysql_db_query($db,$sql,$link);
					$sql2="SELECT s_asig FROM progtareassemanal WHERE s_asig='0' GROUP BY s_asig ORDER BY IdProgTarea ASC";
					$result2=mysql_db_query($db,$sql2,$link);
					$row2=mysql_fetch_array($result2);
					if($row2){echo "<option value=\"0\" selected>Todos</option>";}
					while($row=mysql_fetch_array($result))
					{	
						$nom_asig=nom_comp($row['s_asig']);
						echo "<option value=\"$row[s_asig]\">$nom_asig</option>";
					}
				?>
              </select>
              <?php }?>
...
{        
 	 irapagina("ver_lista_tareassemanal_pre.php?selec="+valor);
}
<?php
	if ($msg=="1") 
	{
