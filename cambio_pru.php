...
if (isset($retornar)){header("location: lista_prueba.php");}
...
{	
	...
	if($tp_e[1]==$ha)
	{
		...
		$tpo=SanitizeString($tpo);
		$sql6="INSERT INTO cambios_prueba (id_pru,id_orden,fecha_reg,hora_reg,fecha_pru,obj_pru,obs_pru0,obs_pru1,tipo_pru) ".
		"VALUES('$cod','$orden','".date("Y-m-d")."','".date("H:i:s")."','$fecha_r','$obj_pru','$obs_pru0','$obs_pru1','$tpo')";
		mysql_db_query($db,$sql6,$link);

		header("location: cambio_pru2.php?orden=$orden&tp=$tp&num=$cod");
	}
	else
    ...
    <tr> 
      <th>EJECUCION - PRUEBAS 
        <?php	$tp2=md5($orden);
			...
			{
				echo "USUARIAS"; $tpo="1"; $sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba_planif WHERE id_pru='$cod'";
			}
			elseif($tp=="7d428547b8723786029735cde7b75d7d_".$tp2)
			{
				echo "DE SISTEMAS"; $tpo="2"; $sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba_planif WHERE id_pru='$cod'";
			}
			elseif($tp=="f4b29b15eee509a8fa39344471561d29_".$tp2)
			{
				echo "DE SEGURIDAD"; $tpo="3"; $sql_ed="SELECT *, DATE_FORMAT(fecha_reg, '%d/%m/%Y') AS fecha_reg FROM cambios_prueba_planif WHERE id_pru='$cod'";
			}
  ...
  <tr> 
    ...
    <tr align="center"> 
	  ...
	  <?php
		...
		$sql_a1 = "SELECT asig FROM asignacion WHERE id_asig='$row_a[asig]'";
